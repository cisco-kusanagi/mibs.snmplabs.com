...
# Produced by pysmi-0.3.4 at Wed May  1 13:16:18 2019
...
if mibBuilder.loadTexts: configPortDuplexOper.setDescription("The port's operating duplex state.")
...
if mibBuilder.loadTexts: configPortDataRateOper.setDescription("The port's operating transmission data rate.")
...
if mibBuilder.loadTexts: configPortBroadcastControl.setDescription("The port's broadcast packets per second threshold. If the amount of broadcast packets exceeds this threshold, those packets will be dropped.")
...
if mibBuilder.loadTexts: configDiffServDSCP.setDescription('A DSCP value (0 - 63).')
...
if mibBuilder.loadTexts: configUserName.setDescription('The users login Id.')
...
if mibBuilder.loadTexts: configPasswordProtection.setDescription('Determines if a user must login before gaining access to the device.')
...
if mibBuilder.loadTexts: configPasswordEncryption.setDescription('Determines when the password is written to the configuration file whether it is written as clear text or encrypted format.')
...
if mibBuilder.loadTexts: configAuthServerIpAddress.setDescription('The IP Address of the server used for 802.1x authentication.')
...
if mibBuilder.loadTexts: configAuthServerSharedSecret.setDescription('A string of characters used in the RADIUS password encryption process by 802.1x authentication.')
...
if mibBuilder.loadTexts: vlanId.setDescription('The VLAN identifier. A maximum of 64 VLANs is supported. The value must be from 1 to 4094.')
...
mibBuilder.exportSymbols("FSM726S-MIB", configPortAutoNegotiation=configPortAutoNegotiation, netgearPortTrunkGroup=netgearPortTrunkGroup, hostComm=hostComm, configAuthServerSharedSecret=configAuthServerSharedSecret, configAuthServerIpAddress=configAuthServerIpAddress, hostStatus=hostStatus, configVerBootRomImage=configVerBootRomImage, configPasswordAdmin=configPasswordAdmin, configPortDuplex=configPortDuplex, configIpFilter=configIpFilter, vlanPortVlanId=vlanPortVlanId, configGateway=configGateway, vlanPortStatus=vlanPortStatus, configPasswordProtection=configPasswordProtection, configPortDefaultVlanId=configPortDefaultVlanId, configVerHwChipSet=configVerHwChipSet, configBootImageFileName=configBootImageFileName, configMirroringOnOff=configMirroringOnOff, MacAddress=MacAddress, vlanStatus=vlanStatus, configTftpServerFileName=configTftpServerFileName, miscStatisticsReset=miscStatisticsReset, configPortGBIC=configPortGBIC, configDiffServTable=configDiffServTable, vlanEntry=vlanEntry, vlanPortEntry=vlanPortEntry, configPortDataRate=configPortDataRate, commSet=commSet, vlanPortTable=vlanPortTable, netgearCommGroup=netgearCommGroup, commTable=commTable, configHighPriorityOptimization=configHighPriorityOptimization, hostEntry=hostEntry, vlanTable=vlanTable, configDiffServDSCP=configDiffServDSCP, miscReset=miscReset, netgear=netgear, configWebOnOff=configWebOnOff, configNetMask=configNetMask, configRestoreDefaults=configRestoreDefaults, portTrunkEntry=portTrunkEntry, configPort=configPort, netgearConfigGroup=netgearConfigGroup, configPortFlowControlOper=configPortFlowControlOper, portTrunkMemberValue=portTrunkMemberValue, configUserAuthenticationMode=configUserAuthenticationMode, commEntry=commEntry, vlanName=vlanName, configDiffServEntry=configDiffServEntry, vlanPortPortId=vlanPortPortId, configPortFastLink=configPortFastLink, configPortStateOper=configPortStateOper, configIpFilterTable=configIpFilterTable, configTelnetConsole=configTelnetConsole, netgearSpanGroup=netgearSpanGroup, configIpFilterStatus=configIpFilterStatus, OwnerString=OwnerString, configPortTable=configPortTable, configUserName=configUserName, netgearVlanGroup=netgearVlanGroup, hostTable=hostTable, configMirrorSrc=configMirrorSrc, spanOnOff=spanOnOff, configPortPriority=configPortPriority, configPasswordEncryption=configPasswordEncryption, commTrap=commTrap, configBootMode=configBootMode, configVerSwPrimary=configVerSwPrimary, configPhysAddress=configPhysAddress, netgearHostGroup=netgearHostGroup, configBootFtpServerIp=configBootFtpServerIp, hostName=hostName, configMirrorMon=configMirrorMon, hostIndex=hostIndex, configPortComments=configPortComments, portTrunkTable=portTrunkTable, configDiffServPriority=configDiffServPriority, configPortBroadcastControl=configPortBroadcastControl, configIpAddress=configIpAddress, vlanPortTaggedMode=vlanPortTaggedMode, configRadiusSharedSecret=configRadiusSharedSecret, portTrunkId=portTrunkId, portTrunkMember=portTrunkMember, commIndex=commIndex, configIGMPOnOff=configIGMPOnOff, configSave=configSave, commName=commName, configTftpServerIpAddress=configTftpServerIpAddress, fsm726s=fsm726s, configRadiusServerIpAddress=configRadiusServerIpAddress, configTftpOperation=configTftpOperation, configIpFilterEntry=configIpFilterEntry, configPortFlowControl=configPortFlowControl, commStatus=commStatus, configIpFilterIpAddress=configIpFilterIpAddress, hostIP=hostIP, configPortEntry=configPortEntry, netgearMiscGroup=netgearMiscGroup, commGet=commGet, vlanId=vlanId, configPortDuplexOper=configPortDuplexOper, snmpManagedSwitch=snmpManagedSwitch, configPortDataRateOper=configPortDataRateOper, configIpAssignmentMode=configIpAssignmentMode)
