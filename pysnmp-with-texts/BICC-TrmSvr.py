...
# Produced by pysmi-0.3.4 at Wed May  1 11:39:01 2019
...
enterprises, Counter64, Gauge32, MibIdentifier, iso, MibScalar, MibTable, MibTableRow, MibTableColumn, IpAddress, NotificationType, ModuleIdentity, TimeTicks, Unsigned32, Integer32, Counter32, Bits, ObjectIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "Counter64", "Gauge32", "MibIdentifier", "iso", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "IpAddress", "NotificationType", "ModuleIdentity", "TimeTicks", "Unsigned32", "Integer32", "Counter32", "Bits", "ObjectIdentity")
...
if mibBuilder.loadTexts: vSysIdentifier.setDescription('A unique identifier of the server type. The OID is one of those found under { serverDevice }, such as { vcpServerDevice }.')
...
mibBuilder.exportSymbols("BICC-TrmSvr", vTelSessIpPrecedence=vTelSessIpPrecedence, vcpServerMib=vcpServerMib, vTacServerCurrNumber=vTacServerCurrNumber, vArpRetryMax=vArpRetryMax, vProfEraseChar=vProfEraseChar, vPortEntry=vPortEntry, vTelSessRemTcpPort=vTelSessRemTcpPort, vTcpTimerInterval=vTcpTimerInterval, vLtNodeGroups=vLtNodeGroups, vProfEntry=vProfEntry, vLtTotalRcvPkts=vLtTotalRcvPkts, vPortNumber=vPortNumber, vSysReboot=vSysReboot, vQueNodeName=vQueNodeName, vSnmpReadCommunity=vSnmpReadCommunity, vLtSessState=vLtSessState, vProfNullPass=vProfNullPass, vProfDomain=vProfDomain, vProfNop=vProfNop, vLtRcvDuplicatePkts=vLtRcvDuplicatePkts, vPortType=vPortType, vPortSignalCheck=vPortSignalCheck, vPortCurrSessProt=vPortCurrSessProt, vProfEraseLine=vProfEraseLine, vSvcTcpPort=vSvcTcpPort, vQueStatus=vQueStatus, vLtSessTable=vLtSessTable, vPortXonMark=vPortXonMark, vSvcStatus=vSvcStatus, vRlogSessIndex=vRlogSessIndex, vSnmpTrapDestColdEnable=vSnmpTrapDestColdEnable, vSvcName=vSvcName, vTelCourierEnable=vTelCourierEnable, vProfCrFromTerm=vProfCrFromTerm, vSnmpTrapDestEntry=vSnmpTrapDestEntry, vPortPrefNode=vPortPrefNode, vPortSecurityEnable=vPortSecurityEnable, vPortAutoConnect=vPortAutoConnect, vTelSessKeepalive=vTelSessKeepalive, bdn=bdn, vPortRIstate=vPortRIstate, vSnmpTrapDestCurrNumber=vSnmpTrapDestCurrNumber, vPortVirtualEnable=vPortVirtualEnable, vArpMaxEntries=vArpMaxEntries, serverProtocol=serverProtocol, vcpArp=vcpArp, vPortSessionLimit=vPortSessionLimit, vNsRequestMode=vNsRequestMode, vQueMaxEntries=vQueMaxEntries, vPortSpeed=vPortSpeed, vLtNodeName=vLtNodeName, vRlogSessRemTcpPort=vRlogSessRemTcpPort, vLtTotalTrnPkts=vLtTotalTrnPkts, serverAdmin=serverAdmin, vPortName=vPortName, vProfLocalEcho=vProfLocalEcho, vTelSessEntry=vTelSessEntry, vPortHandshake=vPortHandshake, vTelSessPadChar=vTelSessPadChar, vPortBreakDuration=vPortBreakDuration, vPortInactivityTimer=vPortInactivityTimer, vTelSessRemAddr=vTelSessRemAddr, vQueEntry=vQueEntry, vTelSessOrigin=vTelSessOrigin, vLtMaxCkts=vLtMaxCkts, vHostHostname=vHostHostname, vLtSessEntry=vLtSessEntry, vHostStatus=vHostStatus, vRlogSessPortIndex=vRlogSessPortIndex, vcpIp=vcpIp, vRlogSessState=vRlogSessState, vIpBcastAddr=vIpBcastAddr, vcpService=vcpService, vPortXoffMark=vPortXoffMark, vSecIndex=vSecIndex, vProfEndRecord=vProfEndRecord, serverDevice=serverDevice, vcpSnmp=vcpSnmp, vPortCharsDropped=vPortCharsDropped, vProfPadChar=vProfPadChar, vPortPrefSvc=vPortPrefSvc, vProfPadLength=vProfPadLength, vTelSessState=vTelSessState, vSysBootServer=vSysBootServer, vSnmpTrapDestAddr=vSnmpTrapDestAddr, vHostCurrNumber=vHostCurrNumber, terminal_server=terminal_server, vSysEtherType=vSysEtherType, vLtSlotPerCkt=vLtSlotPerCkt, vPortAbsoluteLogout=vPortAbsoluteLogout, vPortProfile=vPortProfile, vPortTacacsEnable=vPortTacacsEnable, vQueTable=vQueTable, vPortModemControl=vPortModemControl, vPortMessageCodes=vPortMessageCodes, vPortOutFlowState=vPortOutFlowState, vLtRcvCorruptPkts=vLtRcvCorruptPkts, vRlogSessLocalTcpPort=vRlogSessLocalTcpPort, vPortConsoleIndex=vPortConsoleIndex, vSecStatus=vSecStatus, vPortDCDstate=vPortDCDstate, vProfTable=vProfTable, vPortFailover=vPortFailover, vTelSessGoAhead=vTelSessGoAhead, vPortAbsoluteTimer=vPortAbsoluteTimer, vTelSessDisconnect=vTelSessDisconnect, vPortTrnChars=vPortTrnChars, vPortTable=vPortTable, vTelSessAbortOutput=vTelSessAbortOutput, vSecTable=vSecTable, vSvcVirtualText=vSvcVirtualText, vIpMaxNetHashEntries=vIpMaxNetHashEntries, vQueSvcName=vQueSvcName, vQueEntryNumber=vQueEntryNumber, vNsAllowLowerCase=vNsAllowLowerCase, vRlogSessLocalAddr=vRlogSessLocalAddr, vNsAddress=vNsAddress, vPortRcvChars=vPortRcvChars, protTelnet=protTelnet, vNsEntry=vNsEntry, vPortCurrSessNumber=vPortCurrSessNumber, vPortIndex=vPortIndex, vcpTcp=vcpTcp, vSnmpTrapDestCommunity=vSnmpTrapDestCommunity, vLtSessOrigin=vLtSessOrigin, vLtMcastEnable=vLtMcastEnable, vSnmpTrapDestAuthEnable=vSnmpTrapDestAuthEnable, vTelSessDataMark=vTelSessDataMark, vLtKeepaliveTimer=vLtKeepaliveTimer, vProfStatus=vProfStatus, vTelSessIpTTL=vTelSessIpTTL, vPortRemoteModify=vPortRemoteModify, vcpRlogin=vcpRlogin, vPortDSRstate=vPortDSRstate, protNone=protNone, vSvcCurrNumber=vSvcCurrNumber, vSvcQueueEnable=vSvcQueueEnable, vcpServerDevice=vcpServerDevice, vTacHostname=vTacHostname, vNsCurrNumber=vNsCurrNumber, vSvcLprEnable=vSvcLprEnable, vLtTotalNodeDiscards=vLtTotalNodeDiscards, vPortVerification=vPortVerification, vPortCTSstate=vPortCTSstate, vSecMask=vSecMask, vPortScreenType=vPortScreenType, vTelSessEraseChar=vTelSessEraseChar, vProfName=vProfName, vHostTTL=vHostTTL, vPortForwSwitch=vPortForwSwitch, vTcpRcvWinSize=vTcpRcvWinSize, vPortIOflush=vPortIOflush, vPortInactiveLogout=vPortInactiveLogout, vSvcIdent=vSvcIdent, vSysPrivPassword=vSysPrivPassword, vTelSessCurrNumber=vTelSessCurrNumber, vTelSessAttention=vTelSessAttention, vcpHostname=vcpHostname, vTacStatus=vTacStatus, vPortLogout=vPortLogout, vPortState=vPortState, vTelSessBreak=vTelSessBreak, vProfRemoteEcho=vProfRemoteEcho, vPortLoginPassword=vPortLoginPassword, vSecCurrNumber=vSecCurrNumber, vTcpSegSize=vTcpSegSize, vLtCktTimer=vLtCktTimer, vPortGroups=vPortGroups, vPortBackSwitch=vPortBackSwitch, vArpIdleTimeout=vArpIdleTimeout, vTelSessNop=vTelSessNop, vNsHostname=vNsHostname, vProfPermHostOnly=vProfPermHostOnly, vPortParityType=vPortParityType, vPortBroadcast=vPortBroadcast, vTelCourierText=vTelCourierText, vProfTcpKeepalive=vProfTcpKeepalive, vArpRetryTimeout=vArpRetryTimeout, vProfCrToNet=vProfCrToNet, vPortParityErrs=vPortParityErrs, vSysBroadband=vSysBroadband, vPortPrompt=vPortPrompt, vProfDataMark=vProfDataMark, vSnmpWriteEnable=vSnmpWriteEnable, vPortDSRlogout=vPortDSRlogout, vPortRing=vPortRing, vPortDTRwait=vPortDTRwait, vTelSessRemoteBinary=vTelSessRemoteBinary, vSysBootFileName=vSysBootFileName, vPortQueueing=vPortQueueing, vSvcVirtualEnable=vSvcVirtualEnable, vSvcProfile=vSvcProfile, vRlogSessTermType=vRlogSessTermType, vLtSessNodeName=vLtSessNodeName, vIpMaxAddr=vIpMaxAddr, vSvcLtEnable=vSvcLtEnable, vQueCurrNumber=vQueCurrNumber, vSvcEntry=vSvcEntry, protRlogin=protRlogin, vQuePortName=vQuePortName, vProfTcpTimeout=vProfTcpTimeout, vLtSessDisconnect=vLtSessDisconnect, vSysRemoteBoot=vSysRemoteBoot, vHostEntry=vHostEntry, vPortLock=vPortLock, vPortLogoutMsgEnable=vPortLogoutMsgEnable, vPortFrameErrs=vPortFrameErrs, vPortSignalCheckEnable=vPortSignalCheckEnable, bicc=bicc, vTelSessEndRecord=vTelSessEndRecord, vTelSessLocalEcho=vTelSessLocalEcho, vSnmpTrapDestStatus=vSnmpTrapDestStatus, vSvcPorts=vSvcPorts, vcpPort=vcpPort, vTelSessCrFromTerm=vTelSessCrFromTerm, vPortUserName=vPortUserName, vcpSystem=vcpSystem, vSecGroups=vSecGroups, vTelSessNullPass=vTelSessNullPass, vPortPasswordEnable=vPortPasswordEnable, vSecAddress=vSecAddress, vProfGoAhead=vProfGoAhead, vTelSessEraseLine=vTelSessEraseLine, vLtTotalRetranPkts=vLtTotalRetranPkts, vPortDTRstate=vPortDTRstate, vRlogSessEntry=vRlogSessEntry, vRlogSessOrigin=vRlogSessOrigin, vPortLoginMsgEnable=vPortLoginMsgEnable, vTelSessIntProcess=vTelSessIntProcess, vSysLtGroupStatus=vSysLtGroupStatus, vPortBroadcastEnable=vPortBroadcastEnable, vPortInFlowState=vPortInFlowState, vProfConcatenate=vProfConcatenate, vNsTable=vNsTable, vIpMaxRoutes=vIpMaxRoutes, vIpMaxInterfaces=vIpMaxInterfaces, vTelSessIndex=vTelSessIndex, vProfCurrNumber=vProfCurrNumber, devices=devices, vSnmpWriteCommunity=vSnmpWriteCommunity, vSvcTelEnable=vSvcTelEnable, vSecEnable=vSecEnable, vTelSessTable=vTelSessTable, vcpNameserver=vcpNameserver, vLtReqRejected=vLtReqRejected, vLtMcastTimer=vLtMcastTimer, vProfLocalBinary=vProfLocalBinary, vLtReqAccepted=vLtReqAccepted, vTelSessTermType=vTelSessTermType, vSysSecondaryBoot=vSysSecondaryBoot, vProfTermType=vProfTermType, vNsTTL=vNsTTL, Character=Character, vSysPasswordLimit=vSysPasswordLimit)
mibBuilder.exportSymbols("BICC-TrmSvr", vProfIpTTL=vProfIpTTL, vLtNodeID=vLtNodeID, vSvcTable=vSvcTable, vcpLt=vcpLt, vLtMaxRetran=vLtMaxRetran, vTelSessUserTimeout=vTelSessUserTimeout, server=server, vLtMaxNodes=vLtMaxNodes, vTacAddress=vTacAddress, vcpIpSecurity=vcpIpSecurity, vPortActiveSessions=vPortActiveSessions, vPortPrefPort=vPortPrefPort, vPortPrefMode=vPortPrefMode, vRlogSessTable=vRlogSessTable, vPortVirtualString=vPortVirtualString, vPortDialup=vPortDialup, vLtSessPortIndex=vLtSessPortIndex, vSysMaintenancePassword=vSysMaintenancePassword, vSvcIpAddr=vSvcIpAddr, vTelSessLocalBinary=vTelSessLocalBinary, vLtSessIndex=vLtSessIndex, vLtSessDestName=vLtSessDestName, vPortLocalSwitch=vPortLocalSwitch, vNsStatus=vNsStatus, vRlogSessRemAddr=vRlogSessRemAddr, vTelSessLocalTcpPort=vTelSessLocalTcpPort, vProfIntProcess=vProfIntProcess, vTacEntry=vTacEntry, vcpProfile=vcpProfile, vArpConfirmTimer=vArpConfirmTimer, vPortAccess=vPortAccess, vLtNumber=vLtNumber, vSvcRatingMode=vSvcRatingMode, vSecEntry=vSecEntry, vHostTable=vHostTable, vSnmpTrapDestTable=vSnmpTrapDestTable, vTelSessLocalAddr=vTelSessLocalAddr, vTelSessPortIndex=vTelSessPortIndex, vRlogSessDisconnect=vRlogSessDisconnect, vProfAbortOutput=vProfAbortOutput, vLtSessSvcName=vLtSessSvcName, vSvcPassword=vSvcPassword, vPortBreakMode=vPortBreakMode, vProfTcpPort=vProfTcpPort, vTcpIpPrecedence=vTcpIpPrecedence, slot=slot, vSvcConnectEnable=vSvcConnectEnable, vLtRcvCorruptMcasts=vLtRcvCorruptMcasts, vcpQueue=vcpQueue, vcpTacacs=vcpTacacs, service=service, vSysPrimaryBoot=vSysPrimaryBoot, vPortOverrunErrs=vPortOverrunErrs, vTelSessPadLength=vTelSessPadLength, vLtMaxSvcs=vLtMaxSvcs, DisplayChar=DisplayChar, vPortRTSstate=vPortRTSstate, vHostAddress=vHostAddress, vcpTelnet=vcpTelnet, vProfIpPrecedence=vProfIpPrecedence, vTelSessRemoteEcho=vTelSessRemoteEcho, vRlogSessCurrNumber=vRlogSessCurrNumber, vPortInterrupts=vPortInterrupts, port=port, vTcpSendQSize=vTcpSendQSize, vPortCharSize=vPortCharSize, vPortFlowType=vPortFlowType, LtGroupList=LtGroupList, vProfAttention=vProfAttention, vSysBootFilePath=vSysBootFilePath, serverMibs=serverMibs, vProfRemoteBinary=vProfRemoteBinary, vTcpChecksumEnable=vTcpChecksumEnable, protLt=protLt, vPortAutoPrompt=vPortAutoPrompt, vIpMaxHostHashEntries=vIpMaxHostHashEntries, vTelSessCrToNet=vTelSessCrToNet, vLtSessCurrNumber=vLtSessCurrNumber, vTacTable=vTacTable, vSnmpReadWriteCommunity=vSnmpReadWriteCommunity, vSysIdentifier=vSysIdentifier, vProfBreak=vProfBreak, vSvcRawEnable=vSvcRawEnable, vPortAutobaud=vPortAutobaud, vSvcRating=vSvcRating)
