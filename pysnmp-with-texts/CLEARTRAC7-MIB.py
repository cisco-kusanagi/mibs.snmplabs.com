...
# Produced by pysmi-0.3.4 at Wed May  1 12:25:04 2019
...
MibScalar, MibTable, MibTableRow, MibTableColumn, ModuleIdentity, ObjectIdentity, NotificationType, Integer32, IpAddress, enterprises, iso, Counter64, Counter32, Gauge32, NotificationType, MibIdentifier, Bits, TimeTicks, Unsigned32 = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ModuleIdentity", "ObjectIdentity", "NotificationType", "Integer32", "IpAddress", "enterprises", "iso", "Counter64", "Counter32", "Gauge32", "NotificationType", "MibIdentifier", "Bits", "TimeTicks", "Unsigned32")
...
if mibBuilder.loadTexts: sysRackId.setDescription('ID of the Rack this board is plugged in')
...
if mibBuilder.loadTexts: sysPosProduct.setDescription('The Product descriptor of the board at sysPosIndex in the OID form e.g.: {lucent cleartrac7 product product-id} ')
...
if mibBuilder.loadTexts: sysDLCI.setDescription('The local DLCI address used as Srce field in the connection request to an ACT SDM-FP')
...
if mibBuilder.loadTexts: sysExtensionNumLength.setDescription('The number of digits that must be used to define a port Extension Number, or a Destination Extension Number in the Voice Mapping Table for this Node.')
...
if mibBuilder.loadTexts: sysVoiceEncoding.setDescription('SDM-9400 and V410 Only...')
...
if mibBuilder.loadTexts: sysVoiceClocking.setDescription('SDM-9400 and V410 Only...')
...
if mibBuilder.loadTexts: sysHuntForwardingADLCI.setDescription('Hunt forwarding group A DLCI address. DLCI address where a busy call for hunt group A will be forwarded on an ACT SDM-FP PVC')
...
if mibBuilder.loadTexts: sysHuntForwardingBDLCI.setDescription('Hunt forwarding group B DLCI address. DLCI address where a busy call for hunt group B will be forwarded on an ACT SDM-FP PVC')
...
if mibBuilder.loadTexts: sysHuntForwardingASvcAddress.setDescription('Hunt forwarding group A SVC address. SVC address where a busy call for hunt group A will be forwarded on an ACT SDM-FP SVC. NOTE: sysHuntForwardingAUnit must be empty otherwise this parameter is ignored')
...
if mibBuilder.loadTexts: sysHuntForwardingBSvcAddress.setDescription('Hunt forwarding group A SVC address. SVC address where a busy call for hunt group A will be forwarded on an ACT SDM-FP SVC. NOTE: sysHuntForwardingBUnit must be empty otherwise this parameter is ignored')
...
if mibBuilder.loadTexts: ifwanPppNegociateAccm.setDescription('This parameter determines whether the port will inform the remote peer that it can enter into Asynchronous Control Character Map (ACCM) negotiation. This option provides a way to negotiate the use of control character mapping on asynchronous links. By default, PPP maps all control characters into an appropriate two-character sequence. This is usually not necessary. The cleartrac uses the ACCM negotiation option to inform the remote end which control characters must remain mapped, and which ones do not need to remain mapped when the remote end sends them. Set this parameter to YES to permit ACCM negotiation. Set it to NO (the default value) to disallow this negotiation process. If you set this parameter to YES, you must also configure the Requested ACCM Characters parameter.')
...
if mibBuilder.loadTexts: ifvceActivationType.setDescription('For each cleartrac voice port, voice/fax line activation can be configured according to one of the following activation types.')
...
if mibBuilder.loadTexts: ifvceSilenceSuppress.setDescription('This parameter specifies the degree to which periods of silence will be suppressed and reduced during transmission. A higher Silence Suppression level increases the attenuation of the line, lowers background noise and reduces overall bandwidth use. ONLY FOR EXPANSION CARDS OTHER THAN DVC')
...
if mibBuilder.loadTexts: ifvceDVCSilenceSuppress.setDescription('This parameter specifies the degree to which periods of silence will be suppressed and reduced during transmission. A higher Silence Suppression level increases the attenuation of the line, lowers background noise and reduces overall bandwidth use. ONLY FOR DVC CARDS')
...
if mibBuilder.loadTexts: ifvceLocalInbound.setDescription('This parameter specifies the local voice level into the port, measured in 1 db increments. ONLY FOR CARDS OTHER THAN DVC')
...
if mibBuilder.loadTexts: ifvceLocalOutbound.setDescription('This parameter specifies the local voice level out of the port, measured in 1 db increments. ONLY FOR CARDS OTHER THAN DVC')
...
if mibBuilder.loadTexts: ifvceDVCLocalInbound.setDescription('This parameter specifies the local voice level into the port, measured in 1 db increments. ONLY FOR DVC CARDS')
...
if mibBuilder.loadTexts: ifvceDVCLocalOutbound.setDescription('This parameter specifies the local voice level out of the port, measured in 1 db increments. ONLY FOR DVC CARDS')
...
if mibBuilder.loadTexts: ifvceLinkDwnBusy.setDescription('Used by SDM-9400 and V410 VFC03 cards and by the SDM-9350 and V352 voice ports for versions 7.1.0 and higher. Used only for DVC cards for versions 7.0.3 and lower. See also ifvceAnalogLinkDwnBusy for versions 7.0.3 and lower.')
...
if mibBuilder.loadTexts: ifvceAnalogLinkDwnBusy.setDescription('Used by SDM-9400 and V410 VFC03 cards and by the SDM-9350 and V352 voice ports for versions 7.0.3 and lower. See ifvceLinkDwnBusy for version 7.1.0 and higher and for DVC cards.')
...
if mibBuilder.loadTexts: schedulePort2.setDescription('')
...
if mibBuilder.loadTexts: schedulePort3.setDescription('')
...
if mibBuilder.loadTexts: schedulePort4.setDescription('')
...
if mibBuilder.loadTexts: schedulePort5.setDescription('')
...
if mibBuilder.loadTexts: schedulePort6.setDescription('')
...
if mibBuilder.loadTexts: schedulePort7.setDescription('')
...
if mibBuilder.loadTexts: schedulePort8.setDescription('')
...
if mibBuilder.loadTexts: classPrefRoute.setDescription('Identifies the name of the cleartrac unit to be used as the next hop for sending transparent or PVC multiplex data over multiple direct links (point-to-point or rack backbone connections), where more than one route is available and all available routes are equal in cost. Assigning a different preferred route to different classes ensures static load balancing of transparent traffic over all routes.')
...
if mibBuilder.loadTexts: ospfGlobalGlobalAreaId.setDescription("This parameter identifies the area to which the cleartrac's global Default IP Address belongs.")
...
if mibBuilder.loadTexts: statIflanTr_ReceiveCongestion.setDescription('This counter contains the number of times the receiver has overrun. Token-Ring only.')
...
if mibBuilder.loadTexts: statIflanEth_OneCollision.setDescription('The number of one collision.')
...
if mibBuilder.loadTexts: statIflanEth_TwoCollisions.setDescription('The number of two collisions.')
...
if mibBuilder.loadTexts: statIflanEth_ThreeAndMoreCol.setDescription('The number of three and more collisions.')
...
if mibBuilder.loadTexts: statIflanEth_DeferredTrans.setDescription('This counter contains the Number of frames deferred before transmission.')
...
if mibBuilder.loadTexts: statIflanEth_ExcessiveCollision.setDescription('Excessive collisions. This counter displays the number of frames aborted during transmission due to an excessive number of collisions. These are frames that have not been transmitted successfully.')
...
if mibBuilder.loadTexts: statIflanEth_LateCollision.setDescription('Late collisions. This counter displays the number of transmit frames with late collision, that is, the number of times that a collision was detected later than 512 bits into the transmitted packet.')
...
if mibBuilder.loadTexts: statIflanEth_FrameCheckSeq.setDescription('Frame check sequence errors. This counter displays the number of frames received on the Ethernet port with FCS errors.')
...
if mibBuilder.loadTexts: statIflanEth_Align.setDescription('Alignment errors. This counter displays the number of incomplete frames received on the Ethernet port that do not pass the CRC check.')
...
if mibBuilder.loadTexts: statIflanEth_CarrierSense.setDescription('Carrier sense errors. This counter indicates the number of frames transmitted with carrier sense errors: either the carrier sense signal from the physical layer interface was not asserted, or it was unasserted during transmission of the frame without collision.')
...
mibBuilder.exportSymbols("CLEARTRAC7-MIB", ifwanIpxSap=ifwanIpxSap, iflanTr_Etr=iflanTr_Etr, ifvceRate8kx1=ifvceRate8kx1, statPuTable=statPuTable, statBootpNbReplyReceived=statBootpNbReplyReceived, sysPosTable=sysPosTable, ifwanTxStart=ifwanTxStart, ifwanEnquiryTimer_s=ifwanEnquiryTimer_s, bridgeTr_Number=bridgeTr_Number, ifwanReportCycle=ifwanReportCycle, sysRacksNr=sysRacksNr, sysDLCI=sysDLCI, pvcDialTimeout=pvcDialTimeout, statTimeNbFrameSent=statTimeNbFrameSent, ifwanPppPeerMruUpTo=ifwanPppPeerMruUpTo, pvcCost=pvcCost, ifvceDVCLocalInbound=ifvceDVCLocalInbound, statPvcTable=statPvcTable, firmwareStored=firmwareStored, puDlsIpDst=puDlsIpDst, proxyIndex=proxyIndex, scheduleEntry=scheduleEntry, statPvcState=statPvcState, statBootpReplyReceivedOnPortBootpc=statBootpReplyReceivedOnPortBootpc, statBootpNbReplySend=statBootpNbReplySend, sysAcceptLoop=sysAcceptLoop, pvcInfoRate=pvcInfoRate, statTxDisconnectMessages=statTxDisconnectMessages, statSystemPeakDecompRate=statSystemPeakDecompRate, accountingFileFull=accountingFileFull, accountingFileOverflow=accountingFileOverflow, statPvcChOverflows=statPvcChOverflows, puNumber=puNumber, ifwanRxFlow=ifwanRxFlow, ifvceExtendedDigitSrc=ifvceExtendedDigitSrc, statIfwanT1E1SEF=statIfwanT1E1SEF, ifwanX25Encapsulation=ifwanX25Encapsulation, ifvcePulseMakeBreak_ms=ifvcePulseMakeBreak_ms, ospfVLinkEntry=ospfVLinkEntry, manualHang=manualHang, ifwanCoding=ifwanCoding, sysVoiceEncoding=sysVoiceEncoding, statIfvceProtocol=statIfvceProtocol, ifwanSvcDisconnectTimeoutT305=ifwanSvcDisconnectTimeoutT305, ipRouterEnable=ipRouterEnable, statIfwanT1E1UAS=statIfwanT1E1UAS, statTimeInvalidPortNumbers=statTimeInvalidPortNumbers, ifvceFaxModemRelay=ifvceFaxModemRelay, puSdlcMaxFrame=puSdlcMaxFrame, timepClientUpdateInterval=timepClientUpdateInterval, statIflanEth_TwoCollisions=statIflanEth_TwoCollisions, statSystemBia=statSystemBia, puActive=puActive, statSvcRestart=statSvcRestart, statRxReleaseCompleteMessages=statRxReleaseCompleteMessages, cleartrac7=cleartrac7, puBnnPvc=puBnnPvc, puRole=puRole, statBridge=statBridge, bridgeTr_MaxHop=bridgeTr_MaxHop, ospfRangeNumber=ospfRangeNumber, statBridgePortState=statBridgePortState, statBridgePortTr_SpecRteFrameOut=statBridgePortTr_SpecRteFrameOut, statBridgePortDestination=statBridgePortDestination, statPvcrRouteIntrf=statPvcrRouteIntrf, ifvceEnableDtmfOnTime=ifvceEnableDtmfOnTime, statIfwanRetries=statIfwanRetries, statBridgePortTr_SpecRteFrameIn=statBridgePortTr_SpecRteFrameIn, ifvceDTalkThreshold=ifvceDTalkThreshold, statBridgeBridgeFrameFiltered=statBridgeBridgeFrameFiltered, ospfVLinkTransitDelay=ospfVLinkTransitDelay, statSystemClearErrorLed=statSystemClearErrorLed, statSystem=statSystem, classEntry=classEntry, bondTrig=bondTrig, pvcUserDlci=pvcUserDlci, statPuMode=statPuMode, statIflanEth_ExcessiveCollision=statIflanEth_ExcessiveCollision, statIfwanTable=statIfwanTable, statIfwanIndex=statIfwanIndex, ipstaticIpDest=ipstaticIpDest, statPuChOverflows=statPuChOverflows, ifwanDigitNumber=ifwanDigitNumber, pvcOspfHelloInt=pvcOspfHelloInt, ifwanMgmtInterface=ifwanMgmtInterface, ifwanGroupPoll=ifwanGroupPoll, ospfGlobalRackAreaId=ospfGlobalRackAreaId, iflanIpRipTxRx=iflanIpRipTxRx, bootpMaxHops=bootpMaxHops, ospfAreaEnable=ospfAreaEnable, ifvceExtNumber=ifvceExtNumber, ifwanProtocol=ifwanProtocol, statIfwanT1E1PCV=statIfwanT1E1PCV, pvcIpRipAuthType=pvcIpRipAuthType, statBootpNbRequestSend=statBootpNbRequestSend, fatalMsg=fatalMsg, statSvcOvrOctets=statSvcOvrOctets, ifwanT1E1Status=ifwanT1E1Status, statIfwanPeakRx=statIfwanPeakRx, ifwanOspfDeadInt=ifwanOspfDeadInt, puBanDa=puBanDa, ifwanMaxFrame=ifwanMaxFrame, ifwanSvcIframeRetransmissionsN200=ifwanSvcIframeRetransmissionsN200, statBootpInvalidOpCodeField=statBootpInvalidOpCodeField, statTimeClientRetransmissions=statTimeClientRetransmissions, pvcOspfAreaId=pvcOspfAreaId, timepServerProtocol=timepServerProtocol, ifwanSvcInactiveTimeoutT203=ifwanSvcInactiveTimeoutT203, puBanBnnSsap=puBanBnnSsap, statSvcBadFrames=statSvcBadFrames, ifvceRate8kx2=ifvceRate8kx2, ifwanPppNegociateAccm=ifwanPppNegociateAccm, sysDefaultIpMask=sysDefaultIpMask, ospfVLink=ospfVLink, statTxframes=statTxframes, ifwan=ifwan, iflanProtocol=iflanProtocol, ospfAreaEntry=ospfAreaEntry, classDefaultClass=classDefaultClass, phoneNumber=phoneNumber, ospfVLinkHelloInt=ospfVLinkHelloInt, ifwanClassNumber=ifwanClassNumber, proxyEntry=proxyEntry, ifwanFallBackSpeed_bps=ifwanFallBackSpeed_bps, proxyDefaultGateway=proxyDefaultGateway, ifwanOspfHelloInt=ifwanOspfHelloInt, ifvceRate6kx3=ifvceRate6kx3, statIfwanModemSignal=statIfwanModemSignal, statGrpTable=statGrpTable, sysPosId=sysPosId, pvcProxyAddr=pvcProxyAddr, statSvcOctetsTx=statSvcOctetsTx, ifwanEntry=ifwanEntry, puLinkRemoteUnit=puLinkRemoteUnit, ifwanBodHang_s=ifwanBodHang_s, pvcBurstInfoRate=pvcBurstInfoRate, statTimeNbRequestSent=statTimeNbRequestSent, ifvceDesc=ifvceDesc, timepTimeZone=timepTimeZone, statIfwanEntry=statIfwanEntry, sysCountry=sysCountry, statGrpEntry=statGrpEntry, statSvcMeanTx=statSvcMeanTx, ifvceRemoteUnit=ifvceRemoteUnit, ospfRangeIndex=ospfRangeIndex, ipstaticNextHop=ipstaticNextHop, ifwanCrc4=ifwanCrc4, puDlsSsap=puDlsSsap, statSvcMeanRx=statSvcMeanRx, ifwanOspfRetransmitInt=ifwanOspfRetransmitInt, slotPortInSlot=slotPortInSlot, statIfwanT1E1ESS=statIfwanT1E1ESS, badDestPort=badDestPort, statRxReleaseMessages=statRxReleaseMessages, statSystemSp=statSystemSp, statBootpReplyWithInvalidGiaddr=statBootpReplyWithInvalidGiaddr, ifwanIndex=ifwanIndex, ifwanMsn2=ifwanMsn2, statSvcError=statSvcError, noTrap=noTrap, sysSnmpTrapIpAddr2=sysSnmpTrapIpAddr2, ifwanPppConfigRestartTimer=ifwanPppConfigRestartTimer, ifwanChExp=ifwanChExp, ipxInternalNetNum=ipxInternalNetNum, ipxfilter=ipxfilter, iflanIpxRip=iflanIpxRip, statIflanTable=statIflanTable, statIfwanCompErrs=statIfwanCompErrs, statTimeInvalidLocalIpAddress=statTimeInvalidLocalIpAddress, intfSlotType=intfSlotType, statIfcemClockState=statIfcemClockState, statPvcError=statPvcError, puXidPuType=puXidPuType, classNumber=classNumber, ifwanPppRequestedAccmChar=ifwanPppRequestedAccmChar, ospfRangeNet=ospfRangeNet, statBridgePortTrspFrameOut=statBridgePortTrspFrameOut, puDlsDsap=puDlsDsap, statPvcInfoSignal=statPvcInfoSignal, ifwanMaxChannels=ifwanMaxChannels, statIflanConnectionStatus=statIflanConnectionStatus, sysPsMonitoring=sysPsMonitoring, statPuChSeqErrs=statPuChSeqErrs, ifwanPppRemoteIpAddress=ifwanPppRemoteIpAddress, statTxSetupMessages=statTxSetupMessages, ifwanT1E1InterBit=ifwanT1E1InterBit, ospfRangeTable=ospfRangeTable, ifvceDVCLocalOutbound=ifvceDVCLocalOutbound, ifwanPppAcceptOldIpAddNeg=ifwanPppAcceptOldIpAddNeg, intfEntry=intfEntry, ifvceLocalOutbound=ifvceLocalOutbound, classIndex=classIndex, ifvceRate4k8x1=ifvceRate4k8x1, pvcPort=pvcPort, ifwanLineCoding=ifwanLineCoding, statIfcemDesc=statIfcemDesc, statIfwanT1E1CSS=statIfwanT1E1CSS, pvcNumber=pvcNumber, statBridgeBridgeDesignatedRoot=statBridgeBridgeDesignatedRoot, statBootpFrameTooSmallToBeABootpFrame=statBootpFrameTooSmallToBeABootpFrame, scheduleTable=scheduleTable, ospfAreaImportASExt=ospfAreaImportASExt, pvcOspfPassword=pvcOspfPassword, statIfwanT1E1SES=statIfwanT1E1SES, bootpIpDestAddr4=bootpIpDestAddr4, sysTransitDelay_s=sysTransitDelay_s, scheduleEndTime=scheduleEndTime, ifvceInterface=ifvceInterface, phoneNextHop=phoneNextHop, statIflanEth_LateCollision=statIflanEth_LateCollision, ifwanPppNegociateIpAddress=ifwanPppNegociateIpAddress, pvcOspfTransitDelay=pvcOspfTransitDelay, statTxRetransmissions=statTxRetransmissions, intfDesc=intfDesc, ifwanRingNumber=ifwanRingNumber, sysPosNr=sysPosNr, pvcDlciAddress=pvcDlciAddress, statAlarmEntry=statAlarmEntry, ospfAreaStubMetric=ospfAreaStubMetric, ipxfilterEnable=ipxfilterEnable, linkUp=linkUp, pvcLlcConnection=pvcLlcConnection, statSystemMeanDecompRate=statSystemMeanDecompRate, schedulePort8=schedulePort8, pvcIpxConnection=pvcIpxConnection, pvcHuntForwardingBUnit=pvcHuntForwardingBUnit, statIflanEntry=statIflanEntry, ifwanIpRip=ifwanIpRip, ifwanRemotePort=ifwanRemotePort, ifwanTxStartPass=ifwanTxStartPass, puLinkRemPu=puLinkRemPu, ipaddrTable=ipaddrTable, ifvceFxoTimeout_s=ifvceFxoTimeout_s, statIfvceState=statIfvceState, ospfAreaAuthType=ospfAreaAuthType, statRxAcknowledgeExpiry=statRxAcknowledgeExpiry, ifvceAnalogLinkDwnBusy=ifvceAnalogLinkDwnBusy, statIfwanT1E1LES=statIfwanT1E1LES, statPvcMeanRx=statPvcMeanRx, statPvcrRouteValid=statPvcrRouteValid, statSvcPeakTx=statSvcPeakTx, ipxRouterEnable=ipxRouterEnable, puSdlcTimeout_ms=puSdlcTimeout_ms, statAlarmDate=statAlarmDate, ifwanOspfTransitDelay=ifwanOspfTransitDelay, statRxSetupMessages=statRxSetupMessages, iflanNumber=iflanNumber, ifvceMaxFaxModemRate=ifvceMaxFaxModemRate, iflanPhysAddr=iflanPhysAddr, statBridgePortTable=statBridgePortTable, ipstaticValid=ipstaticValid, puBanBnnMaxFrame=puBanBnnMaxFrame, ifwanBodCall_s=ifwanBodCall_s, statIfvceDesc=statIfvceDesc)
mibBuilder.exportSymbols("CLEARTRAC7-MIB", filter=filter, scheduleEnable=scheduleEnable, ifvceSilenceSuppress=ifvceSilenceSuppress, statIfvceIndex=statIfvceIndex, statPvcChAborts=statPvcChAborts, pvcPriority=pvcPriority, ipstatic=ipstatic, statTimeNbReplySent=statTimeNbReplySent, puIndex=puIndex, bridgeHelloTime_s=bridgeHelloTime_s, pvcBackupCall_s=pvcBackupCall_s, statMissingControlBlocks=statMissingControlBlocks, ifwanDropSyncCharacter=ifwanDropSyncCharacter, puLlcDa=puLlcDa, pvcMaxFrame=pvcMaxFrame, ifwanRemoteUnit=ifwanRemoteUnit, schedulePort3=schedulePort3, statIfwanFramesRx=statIfwanFramesRx, statPvcProtocol=statPvcProtocol, connectionDown=connectionDown, frLinkDown=frLinkDown, statIfwanT1E1DM=statIfwanT1E1DM, statIfvceTable=statIfvceTable, statPvcrRouteTable=statPvcrRouteTable, ipxfilterIndex=ipxfilterIndex, ifwanSync=ifwanSync, ifvceFwdDigits=ifvceFwdDigits, scheduleDay=scheduleDay, statAlarmArg=statAlarmArg, iflanTr_Monitor=iflanTr_Monitor, ifvceR2ExtendedDigitSrc=ifvceR2ExtendedDigitSrc, ospfAreaNumber=ospfAreaNumber, ifwanSvcReleaseTimeoutT308=ifwanSvcReleaseTimeoutT308, ifwanBodLevel=ifwanBodLevel, pvcIpConnection=pvcIpConnection, statIfwanChOverflows=statIfwanChOverflows, ipx=ipx, statTimeNbReplyReceived=statTimeNbReplyReceived, ospfRangeStatus=ospfRangeStatus, ospfGlobalRouterId=ospfGlobalRouterId, ifwanMode=ifwanMode, ifwanSignaling=ifwanSignaling, ifwanPppConfigRetries=ifwanPppConfigRetries, ifvceBroadcastPvc=ifvceBroadcastPvc, statIfwanOctetsRx=statIfwanOctetsRx, statRxframes=statRxframes, pvcNetworkDlci=pvcNetworkDlci, statIflanSpeed=statIflanSpeed, iflanTable=iflanTable, intfSlotNumber=intfSlotNumber, sysContact=sysContact, lucent=lucent, puEntry=puEntry, statIflanOperatingMode=statIflanOperatingMode, puLlcTr_Routing=puLlcTr_Routing, statGrpSorterOverruns=statGrpSorterOverruns, puLlcWindow=puLlcWindow, ifvceR2CompleteDigit=ifvceR2CompleteDigit, periodEnded=periodEnded, sysHuntForwardingADLCI=sysHuntForwardingADLCI, pvcOspfRetransmitInt=pvcOspfRetransmitInt, statPuCompErrs=statPuCompErrs, statSystemSa=statSystemSa, statTimep=statTimep, ipstaticMask=ipstaticMask, puLlcDsap=puLlcDsap, statIfvceDvcPortInUse=statIfvceDvcPortInUse, ifvceToneEnergyDetec=ifvceToneEnergyDetec, statIflanRetries=statIflanRetries, pvcBackup=pvcBackup, ospfAreaTable=ospfAreaTable, ifwanPppAcceptableAccmChar=ifwanPppAcceptableAccmChar, statPvcrRouteMetric=statPvcrRouteMetric, statTimeNbFrameReceived=statTimeNbFrameReceived, statPvcPeakTx=statPvcPeakTx, ifvceRate4k8x2=ifvceRate4k8x2, statBootp=statBootp, intfNumber=intfNumber, iflanOspfDeadInt=iflanOspfDeadInt, intfTable=intfTable, statPvcSpeed=statPvcSpeed, ifwanDesc=ifwanDesc, statEstablishIndications=statEstablishIndications, statRxBytes=statRxBytes, q922Down=q922Down, statIflanTr_ReceiveCongestion=statIflanTr_ReceiveCongestion, statAlarmModule=statAlarmModule, puBanRouting=puBanRouting, ospfRangeEntry=ospfRangeEntry, statAlarmDesc=statAlarmDesc, sysSnmpTrapIpAddr4=sysSnmpTrapIpAddr4, puXid=puXid, statGrpNumber=statGrpNumber, ifwanLineBuild=ifwanLineBuild, ifwanQsigPbxXy=ifwanQsigPbxXy, bridgePriority=bridgePriority, statBridgePortTr_AllRteFrameIn=statBridgePortTr_AllRteFrameIn, statBridgePortTr_SingleRteFrameIn=statBridgePortTr_SingleRteFrameIn, onePsDown=onePsDown, phoneCost=phoneCost, statQ922counters=statQ922counters, statPvcFramesTx=statPvcFramesTx, ifvceNumber=ifvceNumber, ifwanSvcNetworkAddress=ifwanSvcNetworkAddress, ipstaticEntry=ipstaticEntry, proxyNumber=proxyNumber, statAlarmTable=statAlarmTable, statIfcemEntry=statIfcemEntry, iflanSubnetMask=iflanSubnetMask, iflanIpRip=iflanIpRip, sysExtendedDigitsLength=sysExtendedDigitsLength, ipxfilterSap=ipxfilterSap, pvcMaxChannels=pvcMaxChannels, sysSnmpTrapIpAddr1=sysSnmpTrapIpAddr1, ifwanFallBackSpeedEnable=ifwanFallBackSpeedEnable, statTxIframeQdiscards=statTxIframeQdiscards, iflanIndex=iflanIndex, statIflanEth_OneCollision=statIflanEth_OneCollision, fatalTrap=fatalTrap, statIfvceOverruns=statIfvceOverruns, schedulePort2=schedulePort2, slotSlot=slotSlot, statBridgePortTr_SingleRteFrameOut=statBridgePortTr_SingleRteFrameOut, statBootpRequestReceivedOnPortBootpc=statBootpRequestReceivedOnPortBootpc, bridgeTr_SteSpan=bridgeTr_SteSpan, ifvce=ifvce, puBnnFid=puBnnFid, statBridgePortDesignatedCost=statBridgePortDesignatedCost, pvcRingNumber=pvcRingNumber, ifwanFlowControl=ifwanFlowControl, statPvcOvrOctets=statPvcOvrOctets, ifwanConnTimeout_s=ifwanConnTimeout_s, ifvceDtmfOnTime=ifvceDtmfOnTime, ospfAreaAreaId=ospfAreaAreaId, ospfRangeMask=ospfRangeMask, ospfVLinkTable=ospfVLinkTable, ipxfilterTable=ipxfilterTable, ifwanTransparentClassNumber=ifwanTransparentClassNumber, puLlcDynamicWindow=puLlcDynamicWindow, statTxCallProceedingMessages=statTxCallProceedingMessages, statTxReleaseCompleteMessages=statTxReleaseCompleteMessages, ospfVLinkTransitAreaId=ospfVLinkTransitAreaId, statSvcIndex=statSvcIndex, puBanBnnNw=puBanBnnNw, sysDefaultGateway=sysDefaultGateway, scheduleNumber=scheduleNumber, filterIndex=filterIndex, ipxfilterType=ipxfilterType, ifwanHighPriorityTransparentClass=ifwanHighPriorityTransparentClass, ospfVLinkPassword=ospfVLinkPassword, sysThisPosId=sysThisPosId, puSdlcAddress2=puSdlcAddress2, iflanOspfPassword=iflanOspfPassword, statIfwanBadFrames=statIfwanBadFrames, statSvcTable=statSvcTable, statSystemNa=statSystemNa, ifwanSpeed_bps=ifwanSpeed_bps, ipaddrIfIndex=ipaddrIfIndex, ifvceToneType=ifvceToneType, statIflanEth_CarrierSense=statIflanEth_CarrierSense, statIfvceLastError=statIfvceLastError, setupReset=setupReset, iflanIpRipPassword=iflanIpRipPassword, ifwanMultiframing=ifwanMultiframing, sysDay=sysDay, periodStarted=periodStarted, pvcEntry=pvcEntry, statPvcEntry=statPvcEntry, phoneEntry=phoneEntry, ipaddrNr=ipaddrNr, ifwanTxStartCop=ifwanTxStartCop, ifwanIdleCode=ifwanIdleCode, ifwanSubnetMask=ifwanSubnetMask, puLinkClassNumber=puLinkClassNumber, slotIfIndex=slotIfIndex, statIfvceFaxMode=statIfvceFaxMode, statRxConnectMessages=statRxConnectMessages, ifwanPppRequestMagicNum=ifwanPppRequestMagicNum, statIfwanMeanTx=statIfwanMeanTx, pvcIndex=pvcIndex, frLinkUp=frLinkUp, puDlsIpSrc=puDlsIpSrc, puBanBnnDsap=puBanBnnDsap, pvcIpRipPassword=pvcIpRipPassword, ifwanBackupHang_s=ifwanBackupHang_s, scheduleBeginTime=scheduleBeginTime, statSvcInfoSignal=statSvcInfoSignal, ifvceRemotePort=ifvceRemotePort, puDelayBeforeConn_s=puDelayBeforeConn_s, ifwanIpxRip=ifwanIpxRip, puXidId=puXidId, slotPortInSlotEntry=slotPortInSlotEntry, proxyComm=proxyComm, ifwanDropSyncCounter=ifwanDropSyncCounter, sysHuntForwardingASvcAddress=sysHuntForwardingASvcAddress, iflanSpeed=iflanSpeed, iflanOspfAreaId=iflanOspfAreaId, intfNumInType=intfNumInType, statBridgeBridgeRootCost=statBridgeBridgeRootCost, timepClientUdpRetransmissions=timepClientUdpRetransmissions, iflanDesc=iflanDesc, ospfVLinkIndex=ospfVLinkIndex, statIflanEth_ThreeAndMoreCol=statIflanEth_ThreeAndMoreCol, sysRackId=sysRackId, puTable=puTable, statQ933counters=statQ933counters, puBanBnnWindow=puBanBnnWindow, ifwanTerminating=ifwanTerminating, pvcMode=pvcMode, statReleaseIndications=statReleaseIndications, proxy=proxy, statGrpSorterTimeouts=statGrpSorterTimeouts, slot=slot, iflanIpxNetNum=iflanIpxNetNum, statTxBytes=statTxBytes, ipxfilterNumber=ipxfilterNumber, statIfwanPeakTx=statIfwanPeakTx, statRxCallProceedingMessages=statRxCallProceedingMessages, bothFansUp=bothFansUp, sysDesc=sysDesc, puSdlcPort=puSdlcPort, phonePhoneNumber=phonePhoneNumber, statTxReleaseMessages=statTxReleaseMessages, phoneRemoteUnit=phoneRemoteUnit, statTxStatusMessages=statTxStatusMessages, statPvcOvrFrames=statPvcOvrFrames, intfModuleType=intfModuleType, statIfwanOvrFrames=statIfwanOvrFrames, puMode=puMode, statIflanPeakTx_kbps=statIflanPeakTx_kbps, timepClientUdpTimeout=timepClientUdpTimeout, statPvcrRouteEntry=statPvcrRouteEntry, pvcUp=pvcUp, sysVoiceClocking=sysVoiceClocking, statPvcBadFrames=statPvcBadFrames, ifvceActivationType=ifvceActivationType, ifwanPortToBack=ifwanPortToBack, ifvceBroadcastDir=ifvceBroadcastDir, statIfwanTrspLastError=statIfwanTrspLastError, schedule=schedule, statBridgePortDesignatedPort=statBridgePortDesignatedPort, ifwanTxFlow=ifwanTxFlow, ifwanQsigPbxAb=ifwanQsigPbxAb, ospfGlobal=ospfGlobal, proxyTable=proxyTable, ipstaticTable=ipstaticTable, notMemory=notMemory, sysSpeedDialNumLength=sysSpeedDialNumLength, statBridgeBridgeRootPort=statBridgeBridgeRootPort, ipstaticNumber=ipstaticNumber, statSvcBadOctets=statSvcBadOctets, pu=pu, ifwanPppAcceptIpAddress=ifwanPppAcceptIpAddress, statBridgePortDesignatedBridge=statBridgePortDesignatedBridge)
mibBuilder.exportSymbols("CLEARTRAC7-MIB", statSystemTr_Nan=statSystemTr_Nan, slotPortInSlotTable=slotPortInSlotTable, ifvceRate6kx2=ifvceRate6kx2, ifwanPppNegociatePeerMru=ifwanPppNegociatePeerMru, puLlcTimeout_ms=puLlcTimeout_ms, statIflanEth_Align=statIflanEth_Align, schedulePeriod=schedulePeriod, statPvcMode=statPvcMode, oneOrMoreFanDown=oneOrMoreFanDown, ifvceRate8kx3=ifvceRate8kx3, statPvcMeanTx=statPvcMeanTx, ifvceLinkDwnBusy=ifvceLinkDwnBusy, filterNumber=filterNumber, ospfVLinkDeadInt=ospfVLinkDeadInt, schedulePort5=schedulePort5, puXidFormat=puXidFormat, bootp=bootp, filterTable=filterTable, statIflanProtocol=statIflanProtocol, sysJitterBuf=sysJitterBuf, iflanIpxLanType=iflanIpxLanType, statSvcDlci=statSvcDlci, pvcIpxSap=pvcIpxSap, puSdlcAddress=puSdlcAddress, statGrpIndex=statGrpIndex, ifwanPppRemoteSubnetMask=ifwanPppRemoteSubnetMask, statIfwanBadOctets=statIfwanBadOctets, iflanIpxSap=iflanIpxSap, ifwanDestExtNumber=ifwanDestExtNumber, iflanOspfMetricCost=iflanOspfMetricCost, bridgeMaxAge_s=bridgeMaxAge_s, pvcNetworkPort=pvcNetworkPort, statRxDisconnectMessages=statRxDisconnectMessages, badChecksum=badChecksum, ifwanDialer=ifwanDialer, pvcUserPort=pvcUserPort, statSvcPeakRx=statSvcPeakRx, bridge=bridge, sysPosEntry=sysPosEntry, sysDefaultIpAddr=sysDefaultIpAddr, ospfVLinkNeighborRtrId=ospfVLinkNeighborRtrId, intfType=intfType, sysLinkTimeout_s=sysLinkTimeout_s, ifvceToneOn_ms=ifvceToneOn_ms, statSvcProtocol=statSvcProtocol, intfNumInSlot=intfNumInSlot, puDlsDa=puDlsDa, statBootpHopsLimitExceed=statBootpHopsLimitExceed, bootpIpDestAddr2=bootpIpDestAddr2, ifwanIpRipTxRx=ifwanIpRipTxRx, ifwanDialTimeout_s=ifwanDialTimeout_s, ifvceEntry=ifvceEntry, ospfVLinkRetransmitInt=ospfVLinkRetransmitInt, statIfwanProtocol=statIfwanProtocol, statIfwanT1E1BES=statIfwanT1E1BES, statSvcFramesTx=statSvcFramesTx, ifvceR2BusyDigit=ifvceR2BusyDigit, statBridgePortTr_SegmentDuplicate=statBridgePortTr_SegmentDuplicate, classTable=classTable, intfIndex=intfIndex, pvcPvcClass=pvcPvcClass, statPvcrRouteNextHop=statPvcrRouteNextHop, ifwanSvcStatusTimeoutT322=ifwanSvcStatusTimeoutT322, cardDown=cardDown, ifvceDelDigits=ifvceDelDigits, statPvcIndex=statPvcIndex, linkDown=linkDown, ifwanSfType=ifwanSfType, statBridgePortTr_HopCntExceeded=statBridgePortTr_HopCntExceeded, statRxStatusEnquiryMessages=statRxStatusEnquiryMessages, ipaddr=ipaddr, ifwanSvcCallProceedingTimeoutT310=ifwanSvcCallProceedingTimeoutT310, puBanBnnRetry=puBanBnnRetry, ifwanSfCarrierId=ifwanSfCarrierId, filterEntry=filterEntry, statRxStatusMessages=statRxStatusMessages, sysVoiceHighestPriority=sysVoiceHighestPriority, statIflanBadFlags=statIflanBadFlags, ifwanChannelCompressed=ifwanChannelCompressed, statIfwanSpeed=statIfwanSpeed, ifwanMsn1=ifwanMsn1, ifwanCondLMIPort=ifwanCondLMIPort, ifwanInterface=ifwanInterface, ifvceIndex=ifvceIndex, statBridgeBridgeFrameTimeout=statBridgeBridgeFrameTimeout, ipaddrEntry=ipaddrEntry, ip=ip, statTimeNbRequestReceived=statTimeNbRequestReceived, statSvcFramesRx=statSvcFramesRx, statTimeClientSyncFailures=statTimeClientSyncFailures, ifwanExtNumber=ifwanExtNumber, ifwanChUse=ifwanChUse, statBridgeBridgeAddressDiscard=statBridgeBridgeAddressDiscard, puLlcMaxFrame=puLlcMaxFrame, sysExtensionNumLength=sysExtensionNumLength, statBridgePort=statBridgePort, connectionUp=connectionUp, statIflanPeakRx_kbps=statIflanPeakRx_kbps, sysSnmpTrapIpAddr3=sysSnmpTrapIpAddr3, ipaddrAddr=ipaddrAddr, iflanEth_LinkIntegrity=iflanEth_LinkIntegrity, statIflanMeanRx_kbps=statIflanMeanRx_kbps, iflanOspfRetransmitInt=iflanOspfRetransmitInt, statTxStatusEnquiryMessages=statTxStatusEnquiryMessages, statSvcOctetsRx=statSvcOctetsRx, ifwanBackupCall_s=ifwanBackupCall_s, pvcType=pvcType, sysTrapRackandPos=sysTrapRackandPos, statPvcrRouteAge=statPvcrRouteAge, cardup=cardup, iflan=iflan, ifvceFwdDelay_ms=ifvceFwdDelay_ms, ifwanEncodingLaw=ifwanEncodingLaw, ospfGlobalGlobalAreaId=ospfGlobalGlobalAreaId, ifwanT1E1LoopBack=ifwanT1E1LoopBack, statPuChAborts=statPuChAborts, phone=phone, statIfwanOvrOctets=statIfwanOvrOctets, statIflanMeanTx_kbps=statIflanMeanTx_kbps, sysHuntForwardingAUnit=sysHuntForwardingAUnit, stat=stat, pvcHuntForwardingAUnit=pvcHuntForwardingAUnit, bridgeLanType=bridgeLanType, ifvceToneOff_ms=ifvceToneOff_ms, ifwanTable=ifwanTable, timep=timep, sysUnitRoutingVersion=sysUnitRoutingVersion, puSdlcPort2=puSdlcPort2, statIfwanChAborts=statIfwanChAborts, iflanIpAddress=iflanIpAddress, pvcBrgConnection=pvcBrgConnection, iflanPriority=iflanPriority, statPuEntry=statPuEntry, statPvcChSeqErrs=statPvcChSeqErrs, pvcRemotePvc=pvcRemotePvc, statSvcOvrFrames=statSvcOvrFrames, backupCall=backupCall, statIfwanChSeqErrs=statIfwanChSeqErrs, statBridgePortIndex=statBridgePortIndex, iflanOspfEnable=iflanOspfEnable, statBridgePortEntry=statBridgePortEntry, sysRingVolt=sysRingVolt, statProtocolTimeouts=statProtocolTimeouts, pvcCompression=pvcCompression, iflanMaxFrame=iflanMaxFrame, statBridgeBridgeFrameDiscard=statBridgeBridgeFrameDiscard, ifwanIpAddress=ifwanIpAddress, ifwanTimeout=ifwanTimeout, sysPosProduct=sysPosProduct, manualCall=manualCall, ospfRangeAddToArea=ospfRangeAddToArea, statSvcState=statSvcState, puSdlcRetry=puSdlcRetry, statIflanIndex=statIflanIndex, ipxfilterEntry=ipxfilterEntry, bondDeTrig=bondDeTrig, statSystemAlarmNumber=statSystemAlarmNumber, ifwanSvcAddressType=ifwanSvcAddressType, ifvceRate5k8x1=ifvceRate5k8x1, iflanEntry=iflanEntry, pvcIpxNetNum=pvcIpxNetNum, sysAutoSaveDelay=sysAutoSaveDelay, timepClientProtocol=timepClientProtocol, pvcOspfDeadInt=pvcOspfDeadInt, statIfwanFramesTx=statIfwanFramesTx, ifwanMsn3=ifwanMsn3, statIfwanQ922State=statIfwanQ922State, ifwanOspfEnable=ifwanOspfEnable, statBridgeBridge=statBridgeBridge, ifvceTeTimer_s=ifvceTeTimer_s, ipstaticIndex=ipstaticIndex, statInvalidRxSizes=statInvalidRxSizes, ospfRange=ospfRange, statIfcemTable=statIfcemTable, ifwanModem=ifwanModem, statGrpOutOfSeqErrs=statGrpOutOfSeqErrs, statBridgePortTrspFrameIn=statBridgePortTrspFrameIn, ifvceSignaling=ifvceSignaling, iflanOspfHelloInt=iflanOspfHelloInt, badDestPvc=badDestPvc, ifwanPollDelay_ms=ifwanPollDelay_ms, statPuIndex=statPuIndex, statIfwanT1E1LCV=statIfwanT1E1LCV, statIfwanUnderruns=statIfwanUnderruns, sysDialTimer=sysDialTimer, schedulePort1=schedulePort1, ifwanNumber=ifwanNumber, ifvceLocalInbound=ifvceLocalInbound, ipaddrIndex=ipaddrIndex, ifvceToneDetectRegen_s=ifvceToneDetectRegen_s, ifwanBChannels=ifwanBChannels, statBootpNbRequestReceived=statBootpNbRequestReceived, bothPsUp=bothPsUp, ifvceHuntGroup=ifvceHuntGroup, pvcRemoteUnit=pvcRemoteUnit, pvcSubnetMask=pvcSubnetMask, sysLocation=sysLocation, sysPosIpAddr=sysPosIpAddr, bridgeEnable=bridgeEnable, sysClock=sysClock, pvcIpAddress=pvcIpAddress, statPuConnectionStatus=statPuConnectionStatus, statGrpDestName=statGrpDestName, statPvcrRouteName=statPvcrRouteName, statAlarmTime=statAlarmTime, pvcIpxRip=pvcIpxRip, puBanBnnTimeout_ms=puBanBnnTimeout_ms, statAlarmAlarm=statAlarmAlarm, sysPosRackId=sysPosRackId, ifwanGainLimit=ifwanGainLimit, q922Up=q922Up, statBridgePortTr_FrmLngExceeded=statBridgePortTr_FrmLngExceeded, pvcBroadcastGroup=pvcBroadcastGroup, statIfvceFaxRate=statIfvceFaxRate, puDlsMaxFrame=puDlsMaxFrame, intfSlot=intfSlot, statIfwanBadFlags=statIfwanBadFlags, statTxConnectMessages=statTxConnectMessages, schedulePort6=schedulePort6, ifwanPppAcceptAccmPeer=ifwanPppAcceptAccmPeer, proxyIpMask=proxyIpMask, ifwanPvcNumber=ifwanPvcNumber, statPvcRestart=statPvcRestart, statIflanEth_DeferredTrans=statIflanEth_DeferredTrans, statSystemMeanCompRate=statSystemMeanCompRate, statAlarmIndex=statAlarmIndex, bootpIpDestAddr1=bootpIpDestAddr1, bootpEnable=bootpEnable, statBootpCannotReceiveAndForwardOnTheSamePort=statBootpCannotReceiveAndForwardOnTheSamePort, ospfVLinkEnable=ospfVLinkEnable, classPrefRoute=classPrefRoute, ifwanIdle=ifwanIdle, ifwanPriority=ifwanPriority, ospfGlobalAutoVLink=ospfGlobalAutoVLink, statIfwanState=statIfwanState, iflanCost=iflanCost, phoneTable=phoneTable, statPvcCompErrs=statPvcCompErrs, ifvceTable=ifvceTable, filterDefinition=filterDefinition, schedulePort7=schedulePort7, proxyTrapIpAddr=proxyTrapIpAddr, sysHuntForwardingBUnit=sysHuntForwardingBUnit, sysVoiceClass=sysVoiceClass, ifwanClocking=ifwanClocking, ifwanFrameDelay=ifwanFrameDelay, ifwanOspfMetricCost=ifwanOspfMetricCost, statLinkEstablished=statLinkEstablished, ifwanDsOSpeed_bps=ifwanDsOSpeed_bps, cfgStored=cfgStored, pvcOspfEnable=pvcOspfEnable, ifvceRate6kx1=ifvceRate6kx1, ospfVLinkNumber=ospfVLinkNumber, iflanOspfPriority=iflanOspfPriority)
mibBuilder.exportSymbols("CLEARTRAC7-MIB", sysDate=sysDate, bootpIpDestAddr3=bootpIpDestAddr3, ifwanPppAcceptMagicNum=ifwanPppAcceptMagicNum, statGrp=statGrp, statIfwanDesc=statIfwanDesc, classWeight=classWeight, ifvceRate5k8x2=ifvceRate5k8x2, sysHuntForwardingBSvcAddress=sysHuntForwardingBSvcAddress, statIfwanTrspState=statIfwanTrspState, ipaddrType=ipaddrType, ifwanFraming=ifwanFraming, timepGetServerTimeNow=timepGetServerTimeNow, backupHang=backupHang, statIflanEth_FrameCheckSeq=statIflanEth_FrameCheckSeq, statPvcOctetsTx=statPvcOctetsTx, puLlcRetry=puLlcRetry, ifvceR2Group2Digit=ifvceR2Group2Digit, statIfwanDropInsert=statIfwanDropInsert, statSystemResetCounters=statSystemResetCounters, ifwanSvcSetupTimeoutT303=ifwanSvcSetupTimeoutT303, ifwanOspfAreaId=ifwanOspfAreaId, pvcDown=pvcDown, pvcTable=pvcTable, ifwanCellPacketization=ifwanCellPacketization, ospfRangeEnable=ospfRangeEnable, puSdlcWindow=puSdlcWindow, schedulePort4=schedulePort4, timepDaylightSaving=timepDaylightSaving, pvcIpRip=pvcIpRip, statIfvceUnderruns=statIfvceUnderruns, sysHuntForwardingBDLCI=sysHuntForwardingBDLCI, ifwanIpRipAuthType=ifwanIpRipAuthType, iflanTr_RingNumber=iflanTr_RingNumber, mgmt=mgmt, iflanIpRipAuthType=iflanIpRipAuthType, timepTimeZoneSign=timepTimeZoneSign, statPvcBadOctets=statPvcBadOctets, statIfvceEntry=statIfvceEntry, pvc=pvc, proxyIpAddr=proxyIpAddr, statBridgePortTr_AllRteFrameOut=statBridgePortTr_AllRteFrameOut, filterActive=filterActive, bridgeAgingTime_s=bridgeAgingTime_s, bridgeForwardDelay_s=bridgeForwardDelay_s, pvcTimeout_ms=pvcTimeout_ms, sysBackplaneRipVersion=sysBackplaneRipVersion, pvcOspfMetricCost=pvcOspfMetricCost, statIfwanMeanRx=statIfwanMeanRx, statSvcSpeed=statSvcSpeed, ifwanRetry=ifwanRetry, statIflanBadFrames=statIflanBadFrames, pysmi_class=pysmi_class, statSvcMode=statSvcMode, noMsg=noMsg, pvcRetry=pvcRetry, pvcRemoteFpUnit=pvcRemoteFpUnit, ifwanTxHold_s=ifwanTxHold_s, statSystemPeakCompRate=statSystemPeakCompRate, ifwanPppLocalMru=ifwanPppLocalMru, puLlcSsap=puLlcSsap, ospf=ospf, ifwanFormat=ifwanFormat, statSystemClearAlarms=statSystemClearAlarms, system=system, intf=intf, statIfwanOctetsTx=statIfwanOctetsTx, sysName=sysName, ifwanIpxNetNum=ifwanIpxNetNum, statPvcOctetsRx=statPvcOctetsRx, ifwanGroupAddress=ifwanGroupAddress, ifwanTeiMode=ifwanTeiMode, iflanOspfTransitDelay=iflanOspfTransitDelay, statIfcemIndex=statIfcemIndex, statIfwanRestart=statIfwanRestart, sysRingFreq=sysRingFreq, sysVoiceLog=sysVoiceLog, ifwanPppNegociateLocalMru=ifwanPppNegociateLocalMru, ifwanIpRipPassword=ifwanIpRipPassword, ifwanPppSilent=ifwanPppSilent, statBootpCannotRouteFrame=statBootpCannotRouteFrame, statBridgePortTr_SegmentMismatch=statBridgePortTr_SegmentMismatch, statPvcFramesRx=statPvcFramesRx, ifwanSvcMaxTxTimeoutT200=ifwanSvcMaxTxTimeoutT200, statPvcDlci=statPvcDlci, bridgeStpEnable=bridgeStpEnable, timepServerIpAddress=timepServerIpAddress, pvcBackupHang_s=pvcBackupHang_s, statSvcEntry=statSvcEntry, ifwanDuplex=ifwanDuplex, statIflanEth_Interface=statIflanEth_Interface, ifwanSfMode=ifwanSfMode, ifwanCllm=ifwanCllm, sysPsAndFansMonitoring=sysPsAndFansMonitoring, ifvceSpeedDialNum=ifvceSpeedDialNum, pvcIpRipTxRx=pvcIpRipTxRx, statPvcPeakRx=statPvcPeakRx, ifvceDVCSilenceSuppress=ifvceDVCSilenceSuppress, statIfwanInterface=statIfwanInterface, statTxAcknowledgeExpiry=statTxAcknowledgeExpiry, statQ922errors=statQ922errors, ifvceProtocol=ifvceProtocol, statBridgePortDesignatedRoot=statBridgePortDesignatedRoot, ifwanCompression=ifwanCompression, ospfAreaIndex=ospfAreaIndex, phoneIndex=phoneIndex, ifvceFwdType=ifvceFwdType, ifwanOspfPassword=ifwanOspfPassword, ospfArea=ospfArea)
