...
if getattr(mibBuilder, 'version', (0, 0, 0)) > (4, 4, 0):
    if mibBuilder.loadTexts: advaFspR7Tc.setRevisionsDescriptions(('FSP3000 F7 Release 13.2.2 MIB.', 'FSP3000 F7 Release 13.2.1 MIB.', 'FSP3000 F7 Release 12.2.1 MIB.', 'FSP3000 F7 Release 12.1.1 MIB.', 'FSP3000 F7 Release 11.2 MIB.',))
...
class FspR7AlsMode(TextualConvention, Integer32):
    description = 'Determines the reaction in backward direction in response to a LOS on the same Interface: on a Network Interface connected to the DWDM this is a matter of laser safety'
    status = 'current'
    subtypeSpec = Integer32.subtypeSpec + ConstraintsUnion(SingleValueConstraint(0, 1, 2, 3, 4))
...
class FspR7ErrorFwdMode(TextualConvention, Integer32):
    description = 'Determines the reaction in forward direction in response to a LOS on the interface on the opposite side of the module: may override this behaviour in the interests of laser safety.'
    status = 'current'
    subtypeSpec = Integer32.subtypeSpec + ConstraintsUnion(SingleValueConstraint(0, 1, 2, 3, 4, 5, 6))
...
class FspR7Integer32Caps(TextualConvention, OctetString):
    description = 'A FspR7Integer32Caps specification. 1..4 bytes - minValue(Integer32), 5..8 bytes - maxValue(Integer32).'
    status = 'current'
    displayHint = '4d:4d'
...
class FspR7LagFendState(TextualConvention, OctetString):
    description = "The Partner's state variables for the port, encoded as individual bits, as follows: Bits ordering: - LACP_Activity(0), - LACP_Timeout(1), - Aggregation(2), - Synchronization(3), - Collecting(4), - Distributing(5), - Defaulted(6), - Expired(7)"
    status = 'current'
    subtypeSpec = OctetString.subtypeSpec + ValueSizeConstraint(0, 2)

class FspR7LagIdFend(TextualConvention, OctetString):
    description = "LAG ID of the Partner LAG PORT parameters. Concatination of Operational Key, Priority and port number. Uses 4+2+4 bytes. Each byte is represented with 2 octets string. Port LAG ID Parameters field octets contents ----- ------ -------- 1_______1-4_____Key___________(ASCII e.g. '0x00AA') 2_______5-6_____Port Priority_(ASCII e.g. '0x80') 3_______7-10____Port Number___(ASCII e.g. '0x0002') Parameters of the LAG ID are separated by commas, e.g. '00AA,80,0002'"
    status = 'current'
    subtypeSpec = OctetString.subtypeSpec + ValueSizeConstraint(0, 10)

class FspR7LagPorts(TextualConvention, OctetString):
    description = 'Eeach 4 bytes contain the information about the ETH Client or Network entity. Bytes ordering: - AidName(0) - ShelfNumber(1) - SlotNumber(2) - PortNumber(3)'
    status = 'current'
...
class FspR7LagState(TextualConvention, OctetString):
    description = 'The Actors state variables for the port, encoded as individual bits, as follows: Bits ordering: - LACP_Activity(0), - LACP_Timeout(1), - Aggregation(2), - Synchronization(3), - Collecting(4), - Distributing(5), - Defaulted(6), - Expired(7)'
    status = 'current'
    subtypeSpec = OctetString.subtypeSpec + ValueSizeConstraint(0, 2)

class FspR7LagSysIdFend(TextualConvention, OctetString):
    description = "Partner system part of LAG ID. Concatinated from priority and MAC. System LAG ID Parameters field octets contents ----- ------ -------- 1_______1-4_____System Priority__(ASCII e.g. '0x8000') 2_______5-16____MAC Address______(ASCII e.g. 'AC-DE-48-03-67-80') Parameters of the LAG ID are separated by commas, e.g. '8000,AC-DE-48-03-67-80'"
    status = 'current'
    subtypeSpec = OctetString.subtypeSpec + ValueSizeConstraint(0, 16)

class FspR7LaserDelayTimer(TextualConvention, Integer32):
    description = 'Relevant only when ERRFWD=LSROFF. Enables/disables the possibility to delay turning off and on the laser. This applies to the case when turning off the laser is done as an error forwarding mechanism.'
    status = 'current'
    subtypeSpec = Integer32.subtypeSpec + ConstraintsUnion(SingleValueConstraint(0, 1, 2, 3))
...
class FspR7MonLevel(TextualConvention, Integer32):
    description = 'Indicates the highest level of intrusive/non-intrusive performance monitoring performed.'
    status = 'current'
    subtypeSpec = Integer32.subtypeSpec + ConstraintsUnion(SingleValueConstraint(0, 1, 2, 3, 4, 5, 6, 7, 8))
...
class FspR7NCTRouteType(TextualConvention, Integer32):
    description = 'ROUTE-TYPE = NONE | FIBER | CABLE | BKPLANE | EQPT | PROVND. ROUTE-TYPE parameter indicates the connection type between two ports. If a connection between the two ports is within a module (i.e. Client to Network port), the Route-Type is either provisioned (PROVND) of fixed (EQPT). If the connection is between two modules ports; then the Route-Type is FIBER, or CABLE, or BKPLANE.'
    status = 'current'
    subtypeSpec = Integer32.subtypeSpec + ConstraintsUnion(SingleValueConstraint(0, 1, 2, 3, 4, 5, 6))
...
class FspR7NtpAdminState(TextualConvention, Integer32):
    description = 'The Administrative State of the NTP server'
    status = 'current'
...
class FspR7OperState(TextualConvention, Integer32):
    description = 'The Operational State will be displayed on the GUI with full name values; it will be differently displayed in TL1 syntax according to TL1 display rules.'
    status = 'current'
    subtypeSpec = Integer32.subtypeSpec + ConstraintsUnion(SingleValueConstraint(0, 1, 2, 3, 4))
...
class FspR7SnmpHexString(TextualConvention, OctetString):
    description = 'An octet string containing Hex values and presenting them in Hex format. This textual convention should be used when SnmpAdminString or DisplayString cannot be used, because of DISPLAY-HINT limitations.'
    status = 'current'
    subtypeSpec = OctetString.subtypeSpec + ValueSizeConstraint(0, 255)

class FspR7SnmpLongString(TextualConvention, OctetString):
    description = 'This textual convention should be used when SnmpAdminString or DisplayString cannot be used because of the length limitations.'
    status = 'current'
...
class FspR7TxOffOnTm(TextualConvention, Integer32):
    description = 'Relevant only when ERRFWD=TXOFF. Enables/disables the possibility to delay turning off and on the the transmitter. This applies to the case when turning off the transmitter is done as an error forwarding mechanism.'
    status = 'current'
    subtypeSpec = Integer32.subtypeSpec + ConstraintsUnion(SingleValueConstraint(0, 1, 2, 3))
...
class FspR7Unsigned32Caps(TextualConvention, OctetString):
    description = 'A FspR7Unsigned32Caps specification. 1..4 bytes - minValue(Unsigned32), 5..8 bytes - maxValue(Unsigned32).'
    status = 'current'
    displayHint = '4d:4d'
    subtypeSpec = OctetString.subtypeSpec + ValueSizeConstraint(0, 8)
...
mibBuilder.exportSymbols("ADVA-FSPR7-TC-MIB", FspR7InterfaceFunction=FspR7InterfaceFunction, FspR7Baund=FspR7Baund, FspR7LagFendState=FspR7LagFendState, FspR7NtpAdminState=FspR7NtpAdminState, FspR7FlowControlModeCaps=FspR7FlowControlModeCaps, FspR7DispersionConfig=FspR7DispersionConfig, FspR7Unsigned32Caps=FspR7Unsigned32Caps, FspR7VSessStatus=FspR7VSessStatus, FspR7TifOutputResetCaps=FspR7TifOutputResetCaps, FspR7TransmissionModeCaps=FspR7TransmissionModeCaps, FspR7AutoThresReset=FspR7AutoThresReset, Grade=Grade, FspR7YcableTypeCaps=FspR7YcableTypeCaps, FspR7YesNo=FspR7YesNo, FspR7Topology=FspR7Topology, FspR7OpuPayloadTypeCaps=FspR7OpuPayloadTypeCaps, FspR7EnableDisable=FspR7EnableDisable, FspR7ChannelBandwidthCaps=FspR7ChannelBandwidthCaps, FspR7ConnectorType=FspR7ConnectorType, FspR7DispersionConfigCaps=FspR7DispersionConfigCaps, FspR7IpType=FspR7IpType, FspR7LaserForcedOperationCaps=FspR7LaserForcedOperationCaps, FspR7NewSshHostKey=FspR7NewSshHostKey, FspR7ChannelSpacingCaps=FspR7ChannelSpacingCaps, FspR7LacpTimeout=FspR7LacpTimeout, FspR7PRBSTest=FspR7PRBSTest, FspR7OperState=FspR7OperState, FspR7OpticalFiberTypeCaps=FspR7OpticalFiberTypeCaps, FspR7Stages=FspR7Stages, FspR7VSessChangeReason=FspR7VSessChangeReason, FspR7Scrambling=FspR7Scrambling, FspR7RowStatus=FspR7RowStatus, FspR7NumberOfChannels=FspR7NumberOfChannels, FspR7PmSnapshotParameterTypes=FspR7PmSnapshotParameterTypes, FspR7ChannelBandwidth=FspR7ChannelBandwidth, Counter64StringCaps=Counter64StringCaps, FspR7StateConnection=FspR7StateConnection, FspR7AdminState=FspR7AdminState, FspR7OpticalSubBandCaps=FspR7OpticalSubBandCaps, FspR7PathProt=FspR7PathProt, FspR7TxOffOnTm=FspR7TxOffOnTm, FspR7ColumnMark=FspR7ColumnMark, FspR7RedLinedStateCaps=FspR7RedLinedStateCaps, FspR7TurnupConfigCaps=FspR7TurnupConfigCaps, FspR7FunctionCrsCaps=FspR7FunctionCrsCaps, FspR7StartPmSnapshot=FspR7StartPmSnapshot, FspR7Conn=FspR7Conn, FspR7ScramblingCaps=FspR7ScramblingCaps, FspR7VSessWriteAccess=FspR7VSessWriteAccess, advaFspR7Tc=advaFspR7Tc, FspR7EdfaOutputPowerRating=FspR7EdfaOutputPowerRating, FspR7YesNoCaps=FspR7YesNoCaps, FspR7TypeCrs=FspR7TypeCrs, PYSNMP_MODULE_ID=advaFspR7Tc, FspR7SignalDirection=FspR7SignalDirection, FspR7LagPortTypeCaps=FspR7LagPortTypeCaps, FspR7ChannelNumber=FspR7ChannelNumber, FspR7FunctionCrs=FspR7FunctionCrs, FspR7MaxBitErrorRateCaps=FspR7MaxBitErrorRateCaps, FspR7ManualAutoCaps=FspR7ManualAutoCaps, FspR7EocProtAvailability=FspR7EocProtAvailability, FspR7PrbsPmReset=FspR7PrbsPmReset, FspR7IPv6TypeCaps=FspR7IPv6TypeCaps, FspR7TrafficDirection=FspR7TrafficDirection, FspR7InterfaceTypeCaps=FspR7InterfaceTypeCaps, FspR7DispersionCompensationCaps=FspR7DispersionCompensationCaps, FspR7RlsManCaps=FspR7RlsManCaps, FspR7LacpTimeoutCaps=FspR7LacpTimeoutCaps, FspR7InitEqualization=FspR7InitEqualization, FspR7OscChannel=FspR7OscChannel, FspR7CodeGainCaps=FspR7CodeGainCaps, FspR7FiberBrandCaps=FspR7FiberBrandCaps, FspR7NumberOfChannelsCaps=FspR7NumberOfChannelsCaps, FspR7TypeConnection=FspR7TypeConnection, FspR7PortBehaviour=FspR7PortBehaviour, FspR7Protocol=FspR7Protocol, FspR7TypeConnectionCaps=FspR7TypeConnectionCaps, FspR7PsuOutputPower=FspR7PsuOutputPower, FspR7InitEqualizationCaps=FspR7InitEqualizationCaps, FspR7PortRole=FspR7PortRole, FspR7OpticalFiberType=FspR7OpticalFiberType, FspR7AlsModeCaps=FspR7AlsModeCaps, ConnectionNotation=ConnectionNotation, FspR7PortRoleCaps=FspR7PortRoleCaps, FspR7AutoThresResetCaps=FspR7AutoThresResetCaps, ApsType=ApsType, FspR7APSCommand=FspR7APSCommand, FspR7TelemetryOutputCaps=FspR7TelemetryOutputCaps, FspR7NtpTestStatus=FspR7NtpTestStatus, FspR7OpticalGroupCaps=FspR7OpticalGroupCaps, FspR7SingleFiberLocation=FspR7SingleFiberLocation, FspR7DCFiberType=FspR7DCFiberType, FspR7ParityBit=FspR7ParityBit, FspR7KeyLength=FspR7KeyLength, FspR7RedLinedState=FspR7RedLinedState, FspR7ProtectionType=FspR7ProtectionType, FspR7NdpCleanupCaps=FspR7NdpCleanupCaps, FspR7OspfModeCaps=FspR7OspfModeCaps, FspR7OpticalMultiplexLevel=FspR7OpticalMultiplexLevel, FspR7IpTypeCaps=FspR7IpTypeCaps, FspR7SnmpLongString=FspR7SnmpLongString, FspR7TransmissionMode=FspR7TransmissionMode, FspR7EntityType=FspR7EntityType, FspR7AlarmProfileList=FspR7AlarmProfileList, FspR7ChannelIdentifier=FspR7ChannelIdentifier, FspR7AlsMode=FspR7AlsMode, FspR7LacpMode=FspR7LacpMode, FspR7OptUpdate=FspR7OptUpdate, FspR7BERThresholdSection=FspR7BERThresholdSection, FspR7UntaggedFrames=FspR7UntaggedFrames, FspR7DCFiberTypeCaps=FspR7DCFiberTypeCaps, FspR7XfpDecisionThres=FspR7XfpDecisionThres, FspR7ProtectionTypeCaps=FspR7ProtectionTypeCaps, FspR7ErrorFwdMode=FspR7ErrorFwdMode, FspR7ErrorFwdModeCaps=FspR7ErrorFwdModeCaps, FspR7FanProfile=FspR7FanProfile, FspR7Stuff=FspR7Stuff, FspR7DispersionModes=FspR7DispersionModes, FspR7BidirectionalChannel=FspR7BidirectionalChannel, FspR7CodeGain=FspR7CodeGain, FspR7EnableDisableCaps=FspR7EnableDisableCaps, FspR7BaundCaps=FspR7BaundCaps, FspR7ForcedStatusCaps=FspR7ForcedStatusCaps, FspR7GainRange=FspR7GainRange, FspR7OpticalBandCaps=FspR7OpticalBandCaps, FspR7OpticalSubBand=FspR7OpticalSubBand, FspR7AdminStateCaps=FspR7AdminStateCaps, FspR7SupplyTypeCaps=FspR7SupplyTypeCaps, FspR7PlugDataRateCaps=FspR7PlugDataRateCaps, FspR7Integer32Caps=FspR7Integer32Caps, FspR7RoadmNumberCaps=FspR7RoadmNumberCaps, FspR7LaneGroupInventoryCaps=FspR7LaneGroupInventoryCaps, FspR7IPv6Type=FspR7IPv6Type, FspR7PrbsPmResetCaps=FspR7PrbsPmResetCaps, FspR7LineCodingCaps=FspR7LineCodingCaps, FspR7Gain=FspR7Gain, FspR7CertApplyCaps=FspR7CertApplyCaps, FspR7LaneGroupInventory=FspR7LaneGroupInventory, FspR7ConnectorTypeCaps=FspR7ConnectorTypeCaps, FspR7LacpModeCaps=FspR7LacpModeCaps, FspR7ManualAuto=FspR7ManualAuto, FspR7FecType=FspR7FecType, FspR7ChannelIdentifierCaps=FspR7ChannelIdentifierCaps, FspR7LagIdFend=FspR7LagIdFend, FspR7LagPortType=FspR7LagPortType, FspR7NumberOfChannelsProv=FspR7NumberOfChannelsProv, FspR7OpticalGroup=FspR7OpticalGroup, FspR7EncapsulationMethod=FspR7EncapsulationMethod, FspR7PortModeCaps=FspR7PortModeCaps, FspR7RoadmNumber=FspR7RoadmNumber, FspR7TopologyCaps=FspR7TopologyCaps, FspR7OpticalInterfaceReach=FspR7OpticalInterfaceReach, FspR7OscUsage=FspR7OscUsage, FspR7SnmpHexString=FspR7SnmpHexString, FspR7ValidityPeriod=FspR7ValidityPeriod, FspR7DispersionModesCaps=FspR7DispersionModesCaps, FspR7FltrCableType=FspR7FltrCableType, FspR7NumberOfChannelsProvCaps=FspR7NumberOfChannelsProvCaps, FspR7RenewModeCaps=FspR7RenewModeCaps, FspR7FltrCableTypeCaps=FspR7FltrCableTypeCaps, FspR7Category=FspR7Category, FspR7OtdrPeriod=FspR7OtdrPeriod, FspR7YcableType=FspR7YcableType, FspR7MuxMethodCaps=FspR7MuxMethodCaps, FspR7BERThreshold=FspR7BERThreshold, FspR7FDStatusCaps=FspR7FDStatusCaps, FspR7GainCaps=FspR7GainCaps, FspR7IpMode=FspR7IpMode, FspR7Mapping=FspR7Mapping, FspR7LaserDelayTimerCaps=FspR7LaserDelayTimerCaps, FspR7PRBSTestCaps=FspR7PRBSTestCaps, FspR7NCTRouteType=FspR7NCTRouteType, FspR7ScuRing=FspR7ScuRing, FspR7CertApply=FspR7CertApply, FspR7PmResetCaps=FspR7PmResetCaps, FspR7EdfaOutputPowerRatingCaps=FspR7EdfaOutputPowerRatingCaps, FspR7OscChannelCaps=FspR7OscChannelCaps, FspR7DeploymentScenario=FspR7DeploymentScenario, FspR7EntitySecondaryStates=FspR7EntitySecondaryStates, FspR7LagStandby=FspR7LagStandby, FspR7InvertTelemetryInputLogic=FspR7InvertTelemetryInputLogic, FspR7LossAttenuation=FspR7LossAttenuation, FspR7DispersionCompensation=FspR7DispersionCompensation, FspR7ProtectionRole=FspR7ProtectionRole, FspR7TelemetryOutput=FspR7TelemetryOutput, FspR7MaxBitErrorRate=FspR7MaxBitErrorRate, FspR7ConnCaps=FspR7ConnCaps, FspR7MonLevel=FspR7MonLevel, FspR7CapInventoryCaps=FspR7CapInventoryCaps, FspR7BERThresholdSectionCaps=FspR7BERThresholdSectionCaps, FspR7XfpDecisionThresCaps=FspR7XfpDecisionThresCaps, FspR7NtpTest=FspR7NtpTest, FspR7TypeCrsCaps=FspR7TypeCrsCaps, FspR7FiberBrand=FspR7FiberBrand, FspR7OpuPayloadType=FspR7OpuPayloadType, FspR7ApsChannel=FspR7ApsChannel, FspR7NdpCleanup=FspR7NdpCleanup, FspR7ParityBitCaps=FspR7ParityBitCaps, ApsRevertModeCaps=ApsRevertModeCaps, FspR7NoYesCaps=FspR7NoYesCaps, FspR7SshHostKeyEncryptAlgorithm=FspR7SshHostKeyEncryptAlgorithm, FspR7FanMode=FspR7FanMode, FspR7PathNode=FspR7PathNode, FspR7UntaggedFramesCaps=FspR7UntaggedFramesCaps, ApsTypeCaps=ApsTypeCaps, FspR7NCTraceId=FspR7NCTraceId, FspR7TrapModeLegacy=FspR7TrapModeLegacy, FspR7TiltSetCaps=FspR7TiltSetCaps, FspR7CpAuthTypeCaps=FspR7CpAuthTypeCaps, FspR7NtpSyncStatus=FspR7NtpSyncStatus, FspR7MappingCaps=FspR7MappingCaps, FspR7LaserDelayTimer=FspR7LaserDelayTimer, FspR7AseTabOprCaps=FspR7AseTabOprCaps, FspR7AlarmListType=FspR7AlarmListType, FspR7FDStatus=FspR7FDStatus, FspR7LagState=FspR7LagState, FspR7LossAttenuationCaps=FspR7LossAttenuationCaps, FspR7IpModeCaps=FspR7IpModeCaps, FspR7AseTabOpr=FspR7AseTabOpr, FspR7AidType=FspR7AidType, FspR7EqlzAdminState=FspR7EqlzAdminState, FspR7FiberDetectCaps=FspR7FiberDetectCaps, FspR7PathNodeCaps=FspR7PathNodeCaps, FspR7DisableEnableCaps=FspR7DisableEnableCaps, FspR7PmReset=FspR7PmReset, FspR7InvertTelemetryInputLogicCaps=FspR7InvertTelemetryInputLogicCaps, FspR7EqlzAdminStateCaps=FspR7EqlzAdminStateCaps, FspR7EquipmentAssignState=FspR7EquipmentAssignState, FspR7ConnectState=FspR7ConnectState, FspR7LagPorts=FspR7LagPorts, FspR7StuffCaps=FspR7StuffCaps, FspR7TrafficDirectionCaps=FspR7TrafficDirectionCaps, FspR7TxOffOnTmCaps=FspR7TxOffOnTmCaps, FspR7ForcedStatus=FspR7ForcedStatus, FfpTypeCaps=FfpTypeCaps, FspR7Bitrate=FspR7Bitrate, FspR7LagSysIdFend=FspR7LagSysIdFend, FfpType=FfpType, FspR7OpticalInterfaceReachCaps=FspR7OpticalInterfaceReachCaps, FspR7Ipv6Address=FspR7Ipv6Address, FspR7OptUpdateCaps=FspR7OptUpdateCaps, FspR7ChannelNumberCaps=FspR7ChannelNumberCaps, FspR7LaserForcedOperation=FspR7LaserForcedOperation, FspR7FlowControlMode=FspR7FlowControlMode, FspR7PmSnapshotStatus=FspR7PmSnapshotStatus, FspR7VoaMode=FspR7VoaMode, FspR7KeyLengthCaps=FspR7KeyLengthCaps, FspR7FrameFormat=FspR7FrameFormat, FspR7InterfaceType=FspR7InterfaceType, FspR7RlsMan=FspR7RlsMan, FspR7SingleFiberLocationCaps=FspR7SingleFiberLocationCaps, ApsRevertMode=ApsRevertMode, FspR7GropticsType=FspR7GropticsType, FspR7DisableEnable=FspR7DisableEnable)
mibBuilder.exportSymbols("ADVA-FSPR7-TC-MIB", FspR7BERThresholdCaps=FspR7BERThresholdCaps, FspR7GropticsTypeCaps=FspR7GropticsTypeCaps, FspR7FiberDetect=FspR7FiberDetect, FspR7OspfMode=FspR7OspfMode, FspR7GainRangeCaps=FspR7GainRangeCaps, FspR7TiltSet=FspR7TiltSet, FspR7SupplyType=FspR7SupplyType, EquipmentState=EquipmentState, FspR7CpAuthType=FspR7CpAuthType, EntityClassName=EntityClassName, FspR7PsuOutputPowerCaps=FspR7PsuOutputPowerCaps, FspR7BidirectionalChannelCaps=FspR7BidirectionalChannelCaps, FspR7BitrateCaps=FspR7BitrateCaps, FspR7ForceConfig=FspR7ForceConfig, FspR7ForceConfigCaps=FspR7ForceConfigCaps, FspR7OpticalBand=FspR7OpticalBand, FspR7APSCommandCaps=FspR7APSCommandCaps, FspR7PortBehaviourCaps=FspR7PortBehaviourCaps, FspR7LineCoding=FspR7LineCoding, FspR7ValidityPeriodCaps=FspR7ValidityPeriodCaps, FspR7FecTypeCaps=FspR7FecTypeCaps, FspR7PlugDataRate=FspR7PlugDataRate, FspR7OdtuType=FspR7OdtuType, FspR7MuxMethod=FspR7MuxMethod, FspR7CapInventory=FspR7CapInventory, FspR7OscUsageCaps=FspR7OscUsageCaps, FspR7OtdrPeriodCaps=FspR7OtdrPeriodCaps, FspR7RowStatusCaps=FspR7RowStatusCaps, FspR7SshHostKeyLength=FspR7SshHostKeyLength, FspR7SwitchOverCauseCaps=FspR7SwitchOverCauseCaps, FspR7TifOutputReset=FspR7TifOutputReset, FspR7DeploymentScenarioCaps=FspR7DeploymentScenarioCaps, EntityType=EntityType, FspR7RenewMode=FspR7RenewMode, FspR7TopologyType=FspR7TopologyType, FspR7NoYes=FspR7NoYes, FspR7VoaModeCaps=FspR7VoaModeCaps, FspR7TurnupConfig=FspR7TurnupConfig, FspR7AppType=FspR7AppType, FspR7SwitchOverCause=FspR7SwitchOverCause, FspR7PortMode=FspR7PortMode, FspR7FrameFormatCaps=FspR7FrameFormatCaps, FspR7OpticalLanes=FspR7OpticalLanes, FspR7ChannelSpacing=FspR7ChannelSpacing)
