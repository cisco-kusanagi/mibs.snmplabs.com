...
if getattr(mibBuilder, 'version', (0, 0, 0)) > (4, 4, 0):
    if mibBuilder.loadTexts: fcTc.setRevisionsDescriptions(('Initial version for release.',))
...
if mibBuilder.loadTexts: fcTc.setContactInfo('Fujitsu Technical Assistance Center (FTAC), 1-800-USE-FTAC (1-800-873-3822)')
if mibBuilder.loadTexts: fcTc.setDescription('This MIB module contains common textual conventions (TC) used throughout the Fujitsu Network Communications, Inc. enterprise MIB. It contains only those generic TCs potentially used in more than one MIB module.')
...
mibBuilder.exportSymbols("FSS-COMMON-TC", FCMonType=FCMonType, FCStdObjectIndex=FCStdObjectIndex, FCTimePeriod=FCTimePeriod, FCCondType=FCCondType, FCTcCondType=FCTcCondType, FCStdTypeIndex=FCStdTypeIndex, FCServEffect=FCServEffect, FCObjectName=FCObjectName, FCTrapHistIndex=FCTrapHistIndex, FCTrapType=FCTrapType, FCLocation=FCLocation, FCTcaCondType=FCTcaCondType, FCCondEffect=FCCondEffect, FCSeverity=FCSeverity, FCDirection=FCDirection, PYSNMP_MODULE_ID=fcTc, fcTc=fcTc)
