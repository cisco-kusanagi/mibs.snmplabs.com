...
# Produced by pysmi-0.3.4 at Wed May  1 13:50:41 2019
...
IbGuid, IbSmPortList, infinibandMIB, IbDataPortAndInvalid, IbUnicastLid, IbMulticastLid, IbDataPort = mibBuilder.importSymbols("IB-TC-MIB", "IbGuid", "IbSmPortList", "infinibandMIB", "IbDataPortAndInvalid", "IbUnicastLid", "IbMulticastLid", "IbDataPort")
SnmpAdminString, = mibBuilder.importSymbols("SNMP-FRAMEWORK-MIB", "SnmpAdminString")
ModuleCompliance, ObjectGroup, NotificationGroup = mibBuilder.importSymbols("SNMPv2-CONF", "ModuleCompliance", "ObjectGroup", "NotificationGroup")
Unsigned32, Counter32, NotificationType, Integer32, TimeTicks, Counter64, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn, IpAddress, iso, ModuleIdentity, Gauge32, ObjectIdentity, Bits = mibBuilder.importSymbols("SNMPv2-SMI", "Unsigned32", "Counter32", "NotificationType", "Integer32", "TimeTicks", "Counter64", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "IpAddress", "iso", "ModuleIdentity", "Gauge32", "ObjectIdentity", "Bits")
TextualConvention, TruthValue, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "TruthValue", "DisplayString")
ibSmaMIB = ModuleIdentity((1, 3, 6, 1, 3, 117, 3))
ibSmaMIB.setRevisions(('2005-09-01 12:00',))

if getattr(mibBuilder, 'version', (0, 0, 0)) > (4, 4, 0):
    if mibBuilder.loadTexts: ibSmaMIB.setRevisionsDescriptions(('Initial version published as part of RFC XXXX.',))
...
if mibBuilder.loadTexts: ibSmaMIB.setContactInfo('Sean Harnedy (sharnedy@mangrovesystems.com) Mangrove Systems, Inc. Bhargavi Shah (bshah@fabricnetworks.com) Fabric Networks Corporation Bill Swortwood (Bill.Swortwood@motorola.com) Motorola Corporation Email comments to the IPOIB WG Mailing List at ipoverib@ietf.org.')
if mibBuilder.loadTexts: ibSmaMIB.setDescription('Copyright (C) The Internet Society (2005). The initial version of this MIB module was published in RFC XXXX; for full legal notices see the RFC itself. Supplementary information may be available on http://www.ietf.org/copyrights/ianamib.html. This module contains managed object definitions for the Subnet Management Agent instrumentation for an InfiniBand Subnet Management Agent (SMA).')
...
if mibBuilder.loadTexts: ibSmaNodeType.setDescription('Type of IBA device this SMA is supporting. InfiniBand types are 1: Channel Adapter 2: Switch 3: Router 0,4-255: reserved')
...
if mibBuilder.loadTexts: ibSmaNodeGuid.setDescription('The GUID of this node. The node can be a Host Channel Adpater (HCA), Terminal Channel Adapter (TCA), switch, or router. All ports on the same node shall report the same NodeGUID value. This provides a means for uniquely identifying a node within a subnet and helps to determine the co-location of the ports.')
...
if mibBuilder.loadTexts: ibSmaNodePortGuid.setDescription('The node port GUID. One port within a node can return the NodeGUID as its PortGUID if the port is an integral part of the node and is not field-replaceable.')
...
if mibBuilder.loadTexts: ibSmaNodeRevision.setDescription('Manufacturer assigned device revision. This value shall be the same for all ports on a node.')
...
if mibBuilder.loadTexts: ibSmaNodeVendorId.setDescription('IEEE specified device vendor identifier. This value shall be the same for all ports on a node.')
...
if mibBuilder.loadTexts: ibSmaNodeGid1.setDescription('Global Identifier 1. Used for notification var bind list object. Retrieved from InfiniBand traps 64, 65, 66, 67, 257, 258, and 259. Note: If no Global Routing Header (GRH) is present in the offending packet, this value will be all zeros.')
...
if mibBuilder.loadTexts: ibSmaNodeGid2.setDescription('Global Identifier 2. Used for notification var bind list object. Retrieved from InfiniBand traps 257, 258, and 258. Note: If no GRH is present in the offending packet, this value will be all zeros.')
...
if mibBuilder.loadTexts: ibSmaSwLinearFdbTableNum.setDescription('Number of entries in the Linear Forwarding Data Base. This is the number of entries supported in the Linear Unicast Forwarding Table (starting at LID=0x0000 and going up). The InfiniBand SwitchInfo component is called LinearFDBCap. When the count is 0, this indicates that there is no Linear Forwarding Database present.')
...
if mibBuilder.loadTexts: ibSmaSwRandomFdbTableNum.setDescription('Number of entries in the Random Forwarding Data Base. This is the number of entries supported in the Random Unicast Forwarding Table. The InfiniBand SwitchInfo component is called RandomFDBCap. When the count is 0, this indicates that there is no Random Forwarding Database present.')
...
if mibBuilder.loadTexts: ibSmaSwMulticastFdbTableNum.setDescription('Number of entries in the Multicast Forwarding Data Base. This is the number of entries supported in the Multicast Forwarding Table (starting at LID=0xC000 and going up). The InfiniBand SwitchInfo component is called MulticastFDBCap.')
...
if mibBuilder.loadTexts: ibSmaSwDefaultPort.setDescription('Default forwarding port for unicast packets from the other ports whose DLID does not exist in the random forwarding table. If the port number is unknown, a 0 is returned.')
...
if mibBuilder.loadTexts: ibSmaSwDefMcastPriPort.setDescription('Default forwarding port for multicast packets from the other ports whose DLID does not exist in the forwarding table. If port number is unknown, a 0 is returned.')
...
if mibBuilder.loadTexts: ibSmaSwDefMcastNotPriPort.setDescription('Forward to this port all the multicast packets from the Default Primary port whose DLID does not exist in the forwarding table. If it is unknown, a 0 is returned.')
...
if mibBuilder.loadTexts: ibSmaSwLifeTimeValue.setDescription('Reports the time a packet can live in the switch. This Lifetime Value (LV) is used as a parameter to calculate Switch Lifetime Limit (SLL) value. To calculate a valid SLL, the LV must be between 0 and 19, inclusive. If the LV value is greater than 19, the SLL is to be interpreted as infinite.')
...
if mibBuilder.loadTexts: ibSmaSwPortStateChange.setDescription('This object is set to true(1) whenever the PortState component makes any of the following state changes in the link finite state machine (FSM): Down --> Initialize, Initialize --> Down, Armed --> Down, or Active --> Down')
...
if mibBuilder.loadTexts: ibSmaSwLidsPerPort.setDescription('Specifies the number of LID/Link Mask Control (LMC) combinations that may be assigned to a given external port for switches that support the Random Forwarding table.')
...
if mibBuilder.loadTexts: ibSmaGuidInfoEntry.setDescription('A conceptual row of the ibSmaGuidInfoTable containing information about a particular GUID entry.')
...
if mibBuilder.loadTexts: ibSmaGuidPortIndex.setDescription('Index that identifies IB port associated with this GUID table.')
...
if mibBuilder.loadTexts: ibSmaGuidIndex.setDescription('Index that identifies the GUID entry for this port. For any IB port there can be multiple 64 bit GUIDs assigned. The maximum number is implementation dependent (up to 255). If the port does not have a GUID assigned to it, the row will have 1 entry with a ibSmaGuidVal value of zeroes.')
...
if mibBuilder.loadTexts: ibSmaGuidVal.setDescription('A Global Unique Identifier (GUID) for this port entity.')
...
if mibBuilder.loadTexts: ibSmaPortMKey.setDescription("The 8-byte management key that is kept locally for the management port. If the value is 0's, no authentication is done.")
...
if mibBuilder.loadTexts: ibSmaPortIsSmDisabled.setDescription('Capability flag for this node: Is Subnet Manager Disabled?')
...
if mibBuilder.loadTexts: ibSmaPortMKeyProtectBits.setDescription('Management Key (M_Key) protection bits.')
...
if mibBuilder.loadTexts: ibSmaPortMKeyViolations.setDescription("Counts the number of SMP packets that have been received at this port that have invalid M_Keys. This object is a latching gauge, in that whenever the maximum gauge value is encountered (i.e., the count reaches all 1's), the SMA must reset the gauge to 0, before this object begins incrementing its count again.")
...
if mibBuilder.loadTexts: ibSmaPortPKeyViolations.setDescription("Counts the number of SMP packets that have been received at this port that have invalid P_Keys. This object is a latching gauge, in that whenever the maximum gauge value is encountered (i.e., the count reaches all 1's), the SMA must reset the gauge to 0, before this object begins incrementing its count again.")
...
if mibBuilder.loadTexts: ibSmaPortQKeyViolations.setDescription("Counts the number of SMP packets that have been received at this port that have invalid Q_Keys. This object is a latching gauge, in that whenever the maximum gauge value is encountered (i.e., the count reaches all 1's), the SMA must reset the gauge to 0, before this object begins incrementing its count again.")
...
if mibBuilder.loadTexts: ibSmaPortNumGuid.setDescription('Number of GUID entries supported in the GUIDInfo attribute for this port found in the ibSmaGuidInfoTable.')
...
if mibBuilder.loadTexts: ibSmaPortVirtLaneSupport.setDescription('Virtual Lane configuration supported on this port. Indicated as follows by the SMA: 1: VL0 2: VL0, VL1 3: VL0 - VL3 4: VL0 - VL7 5: VL0 - VL14 0, 6 - 15: reserved.')
...
if mibBuilder.loadTexts: ibSmaPortVlHighPriorityLimit.setDescription('Limit of High Priority component of VL Arbitration Table. This object indicates the amount of high priority packets that can be transmitted without an opportunity to send a low priority packet. Specifically, the number of bytes that can be sent is this value times 4K bytes. A 255 value means the byte limit is unbounded and a value of 0 means that only a single packet from the high-priority table may be sent before an opportunity is given to the low-priority table.')
...
if mibBuilder.loadTexts: ibSmaPortVlArbHighCapacity.setDescription('VL/Weight pairs supported on this port in the VLArbitration table (ibSmaHiPriVlArbTable) for high priority. Shall be 1 to 64 if more than one data VL is supported on this port, 0 otherwise.')
...
if mibBuilder.loadTexts: ibSmaPortVlArbLowCapacity.setDescription('VL/Weight pairs supported on this port in the VLArbitration table (ibSmaLoPriVlArbTable) for low priority. Shall be N to 64 if more than one data VL is supported on this port, 0 otherwise, N being the number of data VLs supported.')
...
if mibBuilder.loadTexts: ibSmaPKeyEntry.setDescription('A conceptual row of the ibSmaPKeyTable containing information about the partition keys.')
...
if mibBuilder.loadTexts: ibSmaPKeyIBAPortIndex.setDescription('The IB data port index for the switch data ports. For switch management port 0 and for CAs and routers, the index value should be 255. This indicates that the entry is not referring to a valid switch data port.')
...
if mibBuilder.loadTexts: ibSmaServiceLevelIndex.setDescription('The Service Level value index for this mapping.')
...
if mibBuilder.loadTexts: ibSmaHiPriIBAPortIndex.setDescription('The index for the configured IBA port.')
...
if mibBuilder.loadTexts: ibSmaLowPriIBAPortIndex.setDescription('The index for the configured IB data port.')
...
if mibBuilder.loadTexts: ibSmaRandomIsValid.setDescription('If true, this LID/Port pair is valid. Note that when the SMA sets this parameter to false(2), entries can be removed.')
...
if mibBuilder.loadTexts: ibSmaSmState.setDescription("Enumerated value indicating this SM's state. The InfiniBand enumeration values are: 0 - not active 1 - discovering 2 - standby 3 - master 4-15 - reserved. Note: a value of reserved returns unknown(5).")
...
if mibBuilder.loadTexts: ibSmaLedIndex.setDescription('Index that identifies the IB data port with its LED.')
...
if mibBuilder.loadTexts: ibSmaExcessBuffOverrunThres.setDescription('This notification translates InfiniBand SMA Trap 130: Excessive Buffer Overrun threshold reached at <LIDADDR><PORTNO>.')
...
mibBuilder.exportSymbols("IB-SMA-MIB", ibSmaPortIsSnmpTunnelSupported=ibSmaPortIsSnmpTunnelSupported, ibSmaGuidInfoTable=ibSmaGuidInfoTable, ibSmaPortIsReinitSupported=ibSmaPortIsReinitSupported, ibSmaPortIsBootMgmtSupported=ibSmaPortIsBootMgmtSupported, ibSmaSmState=ibSmaSmState, ibSmaPortMKeyProtectBits=ibSmaPortMKeyProtectBits, ibSmaLinDestDLIDIndex=ibSmaLinDestDLIDIndex, ibSmaNodeLid2=ibSmaNodeLid2, ibSmaDataPortGroup=ibSmaDataPortGroup, ibSmaFullRouterCACompliance=ibSmaFullRouterCACompliance, ibSmaNodeDataValid=ibSmaNodeDataValid, ibSmaSwLinearFdbTableNum=ibSmaSwLinearFdbTableNum, ibSmaSwLidsPerPort=ibSmaSwLidsPerPort, ibSmaIBAInPortIndex=ibSmaIBAInPortIndex, ibSmaFullSwitchCompliance=ibSmaFullSwitchCompliance, ibSmaObjects=ibSmaObjects, ibSmaPortIsSubnetManager=ibSmaPortIsSubnetManager, ibSmaSwitchInfo=ibSmaSwitchInfo, ibSmaPortMKey=ibSmaPortMKey, ibSmaNodeQueuePair1=ibSmaNodeQueuePair1, ibSmaNodeString=ibSmaNodeString, ibSmaPortVirtLaneSupport=ibSmaPortVirtLaneSupport, ibSmaPortInitTypeContent=ibSmaPortInitTypeContent, ibSmaNodeServiceLevel=ibSmaNodeServiceLevel, ibSmaHiPriWeight=ibSmaHiPriWeight, ibSmaNodeCapMask=ibSmaNodeCapMask, ibSmaPortLinkWidthSupported=ibSmaPortLinkWidthSupported, ibSmaPortIsLedInfoSupported=ibSmaPortIsLedInfoSupported, ibSmaLFTGroup=ibSmaLFTGroup, ibSmaPortLinkWidthEnabled=ibSmaPortLinkWidthEnabled, ibSmaBadPKeyAtSwitchPort=ibSmaBadPKeyAtSwitchPort, ibSmaNodeAttributeId=ibSmaNodeAttributeId, ibSmaPortIsDrNoticeSupported=ibSmaPortIsDrNoticeSupported, ibSmaLowPriVlArbTable=ibSmaLowPriVlArbTable, ibSmaNodeQueuePair2=ibSmaNodeQueuePair2, ibSmaLedIndex=ibSmaLedIndex, ibSmaNodeVendorId=ibSmaNodeVendorId, ibSmaSwDefaultPort=ibSmaSwDefaultPort, ibSmaVendDiagInfoEntry=ibSmaVendDiagInfoEntry, ibSmaRandomForwEgressPort=ibSmaRandomForwEgressPort, ibSmaGuidGroup=ibSmaGuidGroup, ibSmaSubMgrInfo=ibSmaSubMgrInfo, ibSmaPortOverrunErrorThreshold=ibSmaPortOverrunErrorThreshold, ibSmaLedInfoTable=ibSmaLedInfoTable, ibSmaPortInfoTable=ibSmaPortInfoTable, ibSmaSmSmpCount=ibSmaSmSmpCount, ibSmaPortIsTrapSupported=ibSmaPortIsTrapSupported, ibSmaLowPriWeight=ibSmaLowPriWeight, ibSmaNodeLocalPortNumOrZero=ibSmaNodeLocalPortNumOrZero, ibSmaNodeInfo=ibSmaNodeInfo, ibSmaVendDiagInfo=ibSmaVendDiagInfo, ibSmaPKeyMembership=ibSmaPKeyMembership, ibSmaPortMKeyViolations=ibSmaPortMKeyViolations, ibSmaCompliances=ibSmaCompliances, ibSmaPortIsPKeyExtPortTrapSup=ibSmaPortIsPKeyExtPortTrapSup, ibSmaNodeLid=ibSmaNodeLid, ibSmaVendDiagGroup=ibSmaVendDiagGroup, ibSmaSwLinearFdbTop=ibSmaSwLinearFdbTop, ibSmaSL2VLMapTable=ibSmaSL2VLMapTable, ibSmaPortInfoEntry=ibSmaPortInfoEntry, ibSmaPortMasterSmSl=ibSmaPortMasterSmSl, ibSmaPortFilterRawPktInbound=ibSmaPortFilterRawPktInbound, ibSmaPortVlArbHighCapacity=ibSmaPortVlArbHighCapacity, ibSmaMulForEntry=ibSmaMulForEntry, ibSmaSMInfo=ibSmaSMInfo, ibSmaSmSmKey=ibSmaSmSmKey, ibSmaGuidInfoEntry=ibSmaGuidInfoEntry, ibSmaGuidPortIndex=ibSmaGuidPortIndex, ibSmaNodeClassVersion=ibSmaNodeClassVersion, ibSmaPortGenericDiagCode=ibSmaPortGenericDiagCode, ibSmaPortLinkStateChange=ibSmaPortLinkStateChange, ibSmaRFTGroup=ibSmaRFTGroup, ibSmaPortHeadOfQueueLife=ibSmaPortHeadOfQueueLife, ibSmaSlToVlMapInfo=ibSmaSlToVlMapInfo, ibSmaPortIsMKeyNvram=ibSmaPortIsMKeyNvram, ibSmaPortInitTypeResuscitate=ibSmaPortInitTypeResuscitate, ibSmaPortNumGuid=ibSmaPortNumGuid, ibSmaPortMKeyLeasePeriod=ibSmaPortMKeyLeasePeriod, ibSmaRandomLMC=ibSmaRandomLMC, ibSmaLedGroup=ibSmaLedGroup, ibSmaIBAOutPortIndex=ibSmaIBAOutPortIndex, ibSmaVLArbitInfo=ibSmaVLArbitInfo, ibSmaCapabilityMaskModified=ibSmaCapabilityMaskModified, ibSmaLowPriIBAPortIndex=ibSmaLowPriIBAPortIndex, ibSmaPortLid=ibSmaPortLid, ibSmaMulForTable=ibSmaMulForTable, ibSmaPortVendorDiagData=ibSmaPortVendorDiagData, ibSmaPortInitTypeLoad=ibSmaPortInitTypeLoad, ibSmaSwPortStateChange=ibSmaSwPortStateChange, ibSmaPortPhysState=ibSmaPortPhysState, ibSmaConformance=ibSmaConformance, ibSmaPortLocalPhysErrorThreshold=ibSmaPortLocalPhysErrorThreshold, ibSmaHiPriVlArbTable=ibSmaHiPriVlArbTable, ibSmaLowPriVlArbEntry=ibSmaLowPriVlArbEntry, ibSmaPKeyBase=ibSmaPKeyBase, ibSmaRandomForwardingEntry=ibSmaRandomForwardingEntry, ibSmaPortIsPKeyNvram=ibSmaPortIsPKeyNvram, ibSmaMFTInfo=ibSmaMFTInfo, ibSmaPortIsCommManageSupported=ibSmaPortIsCommManageSupported, ibSmaServiceLevelIndex=ibSmaServiceLevelIndex, ibSmaNodeGuid=ibSmaNodeGuid, ibSmaBadQueueKey=ibSmaBadQueueKey, ibSmaNodePartitionTableNum=ibSmaNodePartitionTableNum, ibSmaPortPKeyViolations=ibSmaPortPKeyViolations, ibSmaSL2VLMapEntry=ibSmaSL2VLMapEntry, ibSmaHiPriVlArbEntry=ibSmaHiPriVlArbEntry, ibSmaVirtualLane=ibSmaVirtualLane, ibSmaNodePortGuid=ibSmaNodePortGuid, ibSmaPortIsVendorClassSupported=ibSmaPortIsVendorClassSupported, ibSmaPortVlHighPriorityLimit=ibSmaPortVlHighPriorityLimit, ibSmaBasicNodeCompliance=ibSmaBasicNodeCompliance, ibSmaBadManagementKey=ibSmaBadManagementKey, ibSmaPortLinkWidthActive=ibSmaPortLinkWidthActive, ibSmaNodeNumPorts=ibSmaNodeNumPorts, ibSmaNodeKey=ibSmaNodeKey, ibSmaMulForwMask=ibSmaMulForwMask, ibSmaPKeyTable=ibSmaPKeyTable, ibSmaSwPartitionEnforceNum=ibSmaSwPartitionEnforceNum, ibSmaLedState=ibSmaLedState, ibSmaNotificationsGroup=ibSmaNotificationsGroup, ibSmaPortLinkSpeedEnabled=ibSmaPortLinkSpeedEnabled, ibSmaSwRandomFdbTableNum=ibSmaSwRandomFdbTableNum, ibSmaHiPriVirtLane=ibSmaHiPriVirtLane, ibSmaPortGidPrefix=ibSmaPortGidPrefix, PYSNMP_MODULE_ID=ibSmaMIB, ibSmaNodeBaseVersion=ibSmaNodeBaseVersion, ibSmaSwInboundEnforceCap=ibSmaSwInboundEnforceCap, ibSmaPortIsSmDisabled=ibSmaPortIsSmDisabled, ibSmaMgmtPortGroup=ibSmaMgmtPortGroup, ibSmaGroups=ibSmaGroups, ibSmaPortIsSysImgGuidSupported=ibSmaPortIsSysImgGuidSupported, ibSmaSmInfoTable=ibSmaSmInfoTable, ibSmaPKeyGroup=ibSmaPKeyGroup, ibSmaSwEnhancedPort0=ibSmaSwEnhancedPort0, ibSmaPortFilterRawPktOutbound=ibSmaPortFilterRawPktOutbound, ibSmaPortInitPreservePresReply=ibSmaPortInitPreservePresReply, ibSmaPortResponseTimeValue=ibSmaPortResponseTimeValue, ibSmaLowPriNIndex=ibSmaLowPriNIndex, ibSmaVLArbitGroup=ibSmaVLArbitGroup, ibSmaPortIsDevManageSupported=ibSmaPortIsDevManageSupported, ibSmaSmInfoEntry=ibSmaSmInfoEntry, ibSmaSwDefMcastPriPort=ibSmaSwDefMcastPriPort, ibSmaRandomForwardingPortIndex=ibSmaRandomForwardingPortIndex, ibSmaRandomDestLID=ibSmaRandomDestLID, ibSmaNodeSwitchLid=ibSmaNodeSwitchLid, ibSmaPortInitTypePresence=ibSmaPortInitTypePresence, ibSmaPortMasterSmLid=ibSmaPortMasterSmLid, ibSmaNodeRevision=ibSmaNodeRevision, ibSmaLinForwEgressPort=ibSmaLinForwEgressPort, ibSmaPortLidMaskCount=ibSmaPortLidMaskCount, ibSmaPortNeighborMtu=ibSmaPortNeighborMtu, ibSmaPortIsSlMappingSupported=ibSmaPortIsSlMappingSupported, ibSmaSmGuid=ibSmaSmGuid, ibSmaExcessBuffOverrunThres=ibSmaExcessBuffOverrunThres, ibSmaBadPartitionKey=ibSmaBadPartitionKey, ibSmaHiPriIBAPortIndex=ibSmaHiPriIBAPortIndex, ibSmaPortIsNoticeSupported=ibSmaPortIsNoticeSupported, ibSmaPortPartEnforceInbound=ibSmaPortPartEnforceInbound, ibSmaFlowCntrlUpdateTimerExpire=ibSmaFlowCntrlUpdateTimerExpire, ibSmaPortLinkSpeedActive=ibSmaPortLinkSpeedActive, ibSmaNodeGroup=ibSmaNodeGroup, ibSmaPortQKeyViolations=ibSmaPortQKeyViolations, ibSmaPortPartEnforceOutbound=ibSmaPortPartEnforceOutbound, ibSmaPKeyEntry=ibSmaPKeyEntry, ibSmaSmPriority=ibSmaSmPriority, ibSmaMIB=ibSmaMIB, ibSmaPKeyIndex=ibSmaPKeyIndex, ibSmaSwFilterRawPktOutputCap=ibSmaSwFilterRawPktOutputCap, ibSmaSlToVlMapGroup=ibSmaSlToVlMapGroup, ibSmaGuidInfo=ibSmaGuidInfo, ibSmaPortLinkState=ibSmaPortLinkState, ibSmaNodeMethod=ibSmaNodeMethod, ibSmaPortVendorDiagCode=ibSmaPortVendorDiagCode, ibSmaLedInfoEntry=ibSmaLedInfoEntry, ibSmaGuidVal=ibSmaGuidVal, ibSmaPKeyIBAPortIndex=ibSmaPKeyIBAPortIndex, ibSmaLedInfo=ibSmaLedInfo, ibSmaRFTInfo=ibSmaRFTInfo, ibSmaMulDestDLIDIndex=ibSmaMulDestDLIDIndex, ibSmaPortLinkSpeedSupported=ibSmaPortLinkSpeedSupported, ibSmaPortIsAutoMigrateSupported=ibSmaPortIsAutoMigrateSupported, ibSmaPortIndex=ibSmaPortIndex, ibSmaGuidIndex=ibSmaGuidIndex, ibSmaSmInfoPortIndex=ibSmaSmInfoPortIndex, ibSmaNodePortNum=ibSmaNodePortNum, ibSmaLinForTable=ibSmaLinForTable, ibSmaLinkIntegrityThresReached=ibSmaLinkIntegrityThresReached, ibSmaNodeType=ibSmaNodeType, ibSmaPortInitPreserveContReply=ibSmaPortInitPreserveContReply, ibSmaPortSubnetTimeout=ibSmaPortSubnetTimeout, ibSmaSwOutboundEnforceCap=ibSmaSwOutboundEnforceCap, ibSmaVendDiagPortIndex=ibSmaVendDiagPortIndex, ibSmaMFTGroup=ibSmaMFTGroup, ibSmaMgmtPortInfo=ibSmaMgmtPortInfo, ibSmaPKeyInfo=ibSmaPKeyInfo, ibSmaRandomForwardingTable=ibSmaRandomForwardingTable, ibSmaPortVlArbLowCapacity=ibSmaPortVlArbLowCapacity, ibSmaVendDiagInfoTable=ibSmaVendDiagInfoTable, ibSmaSwitchGroup=ibSmaSwitchGroup, ibSmaPortInitNoLoadReply=ibSmaPortInitNoLoadReply, ibSmaSwMulticastFdbTableNum=ibSmaSwMulticastFdbTableNum, ibSmaSwDefMcastNotPriPort=ibSmaSwDefMcastNotPriPort, ibSmaPortIsCapMaskNoticSupported=ibSmaPortIsCapMaskNoticSupported, ibSmaNotifications=ibSmaNotifications, ibSmaLinForEntry=ibSmaLinForEntry, ibSmaSMGroup=ibSmaSMGroup, ibSmaLowPriVirtLane=ibSmaLowPriVirtLane, ibSmaNodeDeviceId=ibSmaNodeDeviceId, ibSmaNodeGid2=ibSmaNodeGid2, ibSmaPortMtuCapacity=ibSmaPortMtuCapacity, ibSmaSwLifeTimeValue=ibSmaSwLifeTimeValue, ibSmaSysImageGuidModified=ibSmaSysImageGuidModified, ibSmaLFTInfo=ibSmaLFTInfo, ibSmaNodeAttributeModifier=ibSmaNodeAttributeModifier, ibSmaDataPortInfo=ibSmaDataPortInfo, ibSmaNodeGid1=ibSmaNodeGid1, ibSmaPortLinkDownDefaultState=ibSmaPortLinkDownDefaultState, ibSmaPortVlStallCount=ibSmaPortVlStallCount, ibSmaPortVendorDiagIndexFwd=ibSmaPortVendorDiagIndexFwd, ibSmaSwFilterRawPktInputCap=ibSmaSwFilterRawPktInputCap, ibSmaSystemImageGuid=ibSmaSystemImageGuid, ibSmaHiPriNIndex=ibSmaHiPriNIndex, ibSmaPortOperationalVls=ibSmaPortOperationalVls, ibSmaNotificationPrefix=ibSmaNotificationPrefix, ibSmaRandomIsValid=ibSmaRandomIsValid)
