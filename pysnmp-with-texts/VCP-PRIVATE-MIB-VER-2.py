...
# Produced by pysmi-0.3.4 at Wed May  1 15:33:32 2019
...
Gauge32, Unsigned32, IpAddress, Integer32, Counter64, MibScalar, MibTable, MibTableRow, MibTableColumn, ModuleIdentity, iso, ObjectIdentity, Counter32, MibIdentifier, enterprises, NotificationType, TimeTicks, Bits = mibBuilder.importSymbols("SNMPv2-SMI", "Gauge32", "Unsigned32", "IpAddress", "Integer32", "Counter64", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ModuleIdentity", "iso", "ObjectIdentity", "Counter32", "MibIdentifier", "enterprises", "NotificationType", "TimeTicks", "Bits")
...
if mibBuilder.loadTexts: vSysIdentifier.setDescription('A unique identifier of the server type. The OID is one of those found under { dssDevice }, such as { vcp-1000 }.')
...
if mibBuilder.loadTexts: vSysRemoteBoot.setDescription('Indicates whether this server will supply system software as a boot image for remote VCP servers when they request a network boot.')
...
if mibBuilder.loadTexts: vPortCurrSessProt.setDescription('An identifier of which higher-level protocol the current port session is running over. The possibilities are { dssNone } (for no current session, or unknown), { dssTelnet }, { dssRlogin }, or { dssLt }.')
...
mibBuilder.exportSymbols("VCP-PRIVATE-MIB-VER-2", vLtKeepaliveTimer=vLtKeepaliveTimer, vHostTTL=vHostTTL, vSysReboot=vSysReboot, vProfTable=vProfTable, vLtSessOrigin=vLtSessOrigin, vPortActiveSessions=vPortActiveSessions, vSnmpTrapDestAuthEnable=vSnmpTrapDestAuthEnable, vLtRcvCorruptPkts=vLtRcvCorruptPkts, vLtMaxCkts=vLtMaxCkts, dssAdmin=dssAdmin, vRlogSessState=vRlogSessState, vPortTable=vPortTable, vSnmpWriteEnable=vSnmpWriteEnable, vTelSessPadLength=vTelSessPadLength, vPortIndex=vPortIndex, vSvcLprEnable=vSvcLprEnable, vPortLock=vPortLock, vTelSessGoAhead=vTelSessGoAhead, vProfStatus=vProfStatus, vcpIpSecurity=vcpIpSecurity, vLtRcvCorruptMcasts=vLtRcvCorruptMcasts, vPortFailover=vPortFailover, vPortVerification=vPortVerification, vProfNullPass=vProfNullPass, vPortGroups=vPortGroups, vSecMask=vSecMask, vIpMaxRoutes=vIpMaxRoutes, vSysPrimaryBoot=vSysPrimaryBoot, vHostTable=vHostTable, vSecIndex=vSecIndex, vPortBroadcastEnable=vPortBroadcastEnable, port=port, vTelSessTable=vTelSessTable, vHostEntry=vHostEntry, dssDevice=dssDevice, vcpSnmp=vcpSnmp, vNsTable=vNsTable, vRlogSessLocalAddr=vRlogSessLocalAddr, vSvcRatingMode=vSvcRatingMode, vTelSessCrToNet=vTelSessCrToNet, vSvcTcpPort=vSvcTcpPort, vLtCktTimer=vLtCktTimer, vPortDSRstate=vPortDSRstate, vTelSessIpTTL=vTelSessIpTTL, vTelSessEndRecord=vTelSessEndRecord, DisplayChar=DisplayChar, vTelSessEraseLine=vTelSessEraseLine, vHostHostname=vHostHostname, vSvcTable=vSvcTable, vTelSessPortIndex=vTelSessPortIndex, vPortDCDstate=vPortDCDstate, vPortDTRstate=vPortDTRstate, vTelSessDataMark=vTelSessDataMark, vNsCurrNumber=vNsCurrNumber, vPortVirtualEnable=vPortVirtualEnable, vSvcPassword=vSvcPassword, vPortEntry=vPortEntry, vTelSessRemoteEcho=vTelSessRemoteEcho, vRlogSessCurrNumber=vRlogSessCurrNumber, vcpNameserver=vcpNameserver, vcp_200=vcp_200, vSvcIpAddr=vSvcIpAddr, vPortTacacsEnable=vPortTacacsEnable, vArpMaxEntries=vArpMaxEntries, dssLt=dssLt, vPortInterrupts=vPortInterrupts, vSnmpReadCommunity=vSnmpReadCommunity, vLtNumber=vLtNumber, vPortLoginMsgEnable=vPortLoginMsgEnable, vPortRing=vPortRing, vProfAttention=vProfAttention, vArpConfirmTimer=vArpConfirmTimer, vSvcRawEnable=vSvcRawEnable, vTcpSegSize=vTcpSegSize, vPortAbsoluteLogout=vPortAbsoluteLogout, vTcpChecksumEnable=vTcpChecksumEnable, vSvcRating=vSvcRating, vQueNodeName=vQueNodeName, vSvcPorts=vSvcPorts, vTelSessUserTimeout=vTelSessUserTimeout, vPortCharSize=vPortCharSize, dssProtocol=dssProtocol, vTelCourierText=vTelCourierText, vLtTotalNodeDiscards=vLtTotalNodeDiscards, vSnmpTrapDestAddr=vSnmpTrapDestAddr, datability=datability, vSvcConnectEnable=vSvcConnectEnable, vLtTotalRcvPkts=vLtTotalRcvPkts, vTelCourierEnable=vTelCourierEnable, vLtSessEntry=vLtSessEntry, vPortXonMark=vPortXonMark, vPortCTSstate=vPortCTSstate, vcpRlogin=vcpRlogin, vProfEndRecord=vProfEndRecord, vSysEtherType=vSysEtherType, vSvcCurrNumber=vSvcCurrNumber, vPortParityType=vPortParityType, vProfTcpKeepalive=vProfTcpKeepalive, vTelSessIndex=vTelSessIndex, dssRlogin=dssRlogin, vTelSessAbortOutput=vTelSessAbortOutput, vTelSessLocalTcpPort=vTelSessLocalTcpPort, vRlogSessPortIndex=vRlogSessPortIndex, vTelSessPadChar=vTelSessPadChar, vSvcStatus=vSvcStatus, vProfConcatenate=vProfConcatenate, vTelSessEntry=vTelSessEntry, vPortMessageCodes=vPortMessageCodes, vPortScreenType=vPortScreenType, vSysLtGroupStatus=vSysLtGroupStatus, vcpMib=vcpMib, vPortSessionLimit=vPortSessionLimit, vSysBroadband=vSysBroadband, vIpMaxNetHashEntries=vIpMaxNetHashEntries, vQueMaxEntries=vQueMaxEntries, vProfTermType=vProfTermType, vLtMaxRetran=vLtMaxRetran, vPortRemoteModify=vPortRemoteModify, vRlogSessIndex=vRlogSessIndex, vNsAddress=vNsAddress, vSnmpTrapDestColdEnable=vSnmpTrapDestColdEnable, vPortVirtualString=vPortVirtualString, vPortCharsDropped=vPortCharsDropped, vTacAddress=vTacAddress, vTelSessKeepalive=vTelSessKeepalive, vcpLt=vcpLt, vPortAbsoluteTimer=vPortAbsoluteTimer, vRlogSessDisconnect=vRlogSessDisconnect, vProfGoAhead=vProfGoAhead, vQueEntryNumber=vQueEntryNumber, vLtMcastTimer=vLtMcastTimer, vcpHostname=vcpHostname, vPortAutoConnect=vPortAutoConnect, vPortRTSstate=vPortRTSstate, vcpService=vcpService, vQueStatus=vQueStatus, vTelSessTermType=vTelSessTermType, vSvcEntry=vSvcEntry, vcpIp=vcpIp, vPortLogoutMsgEnable=vPortLogoutMsgEnable, vSvcLtEnable=vSvcLtEnable, vQuePortName=vQuePortName, vProfName=vProfName, vPortDialup=vPortDialup, vNsStatus=vNsStatus, vSecAddress=vSecAddress, vSysPrivPassword=vSysPrivPassword, vSvcProfile=vSvcProfile, vPortFrameErrs=vPortFrameErrs, vTelSessBreak=vTelSessBreak, vLtNodeID=vLtNodeID, vPortIOflush=vPortIOflush, vcp_300=vcp_300, vPortQueueing=vPortQueueing, vPortAutobaud=vPortAutobaud, vSnmpTrapDestStatus=vSnmpTrapDestStatus, vPortLogout=vPortLogout, vPortOutFlowState=vPortOutFlowState, vPortBreakMode=vPortBreakMode, vLtSessDestName=vLtSessDestName, vcpTacacs=vcpTacacs, vSvcQueueEnable=vSvcQueueEnable, vTelSessLocalAddr=vTelSessLocalAddr, vSnmpTrapDestTable=vSnmpTrapDestTable, vPortPrefNode=vPortPrefNode, vcp_1000=vcp_1000, vProfLocalBinary=vProfLocalBinary, vTacStatus=vTacStatus, vTcpIpPrecedence=vTcpIpPrecedence, vSecTable=vSecTable, vPortPrefPort=vPortPrefPort, vPortRIstate=vPortRIstate, vSvcIdent=vSvcIdent, vLtReqRejected=vLtReqRejected, vProfPermHostOnly=vProfPermHostOnly, vPortPrefMode=vPortPrefMode, vTelSessEraseChar=vTelSessEraseChar, vProfAbortOutput=vProfAbortOutput, vLtSlotPerCkt=vLtSlotPerCkt, vSysIdentifier=vSysIdentifier, vPortFlowType=vPortFlowType, vRlogSessEntry=vRlogSessEntry, vProfIpTTL=vProfIpTTL, vPortRcvChars=vPortRcvChars, vPortForwSwitch=vPortForwSwitch, vcpProfile=vcpProfile, vProfLocalEcho=vProfLocalEcho, vPortName=vPortName, vProfDomain=vProfDomain, vTacServerCurrNumber=vTacServerCurrNumber, vTelSessRemoteBinary=vTelSessRemoteBinary, vLtTotalTrnPkts=vLtTotalTrnPkts, dssNone=dssNone, vProfEraseChar=vProfEraseChar, vSvcVirtualEnable=vSvcVirtualEnable, vPortBackSwitch=vPortBackSwitch, vHostStatus=vHostStatus, vTelSessCrFromTerm=vTelSessCrFromTerm, vPortDSRlogout=vPortDSRlogout, dssMibs=dssMibs, vSysPasswordLimit=vSysPasswordLimit, vPortCurrSessProt=vPortCurrSessProt, vLtSessCurrNumber=vLtSessCurrNumber, vIpMaxAddr=vIpMaxAddr, vSnmpTrapDestCurrNumber=vSnmpTrapDestCurrNumber, vSnmpTrapDestCommunity=vSnmpTrapDestCommunity, vSecCurrNumber=vSecCurrNumber, vSecStatus=vSecStatus, vPortCurrSessNumber=vPortCurrSessNumber, vcpArp=vcpArp, vLtMaxNodes=vLtMaxNodes, vLtSessState=vLtSessState, vNsEntry=vNsEntry, slot=slot, vPortInactiveLogout=vPortInactiveLogout, vArpRetryMax=vArpRetryMax, vRlogSessLocalTcpPort=vRlogSessLocalTcpPort, vTelSessIntProcess=vTelSessIntProcess, vPortType=vPortType, vProfNop=vProfNop, vProfPadLength=vProfPadLength, Character=Character, vIpBcastAddr=vIpBcastAddr, vPortAccess=vPortAccess, vPortSignalCheckEnable=vPortSignalCheckEnable, vHostAddress=vHostAddress, vQueSvcName=vQueSvcName, vTelSessLocalEcho=vTelSessLocalEcho, vProfTcpTimeout=vProfTcpTimeout, vPortLocalSwitch=vPortLocalSwitch, vLtRcvDuplicatePkts=vLtRcvDuplicatePkts, vSvcTelEnable=vSvcTelEnable, vSysBootServer=vSysBootServer, vArpIdleTimeout=vArpIdleTimeout, vcpPort=vcpPort, vNsRequestMode=vNsRequestMode, vSnmpTrapDestEntry=vSnmpTrapDestEntry, vTelSessLocalBinary=vTelSessLocalBinary, vSysBootFileName=vSysBootFileName, vPortSecurityEnable=vPortSecurityEnable, vLtSessPortIndex=vLtSessPortIndex, vPortXoffMark=vPortXoffMark, vIpMaxHostHashEntries=vIpMaxHostHashEntries, vSvcName=vSvcName, vRlogSessTermType=vRlogSessTermType, vPortBreakDuration=vPortBreakDuration, vcpTelnet=vcpTelnet, vQueEntry=vQueEntry, vTelSessNop=vTelSessNop, vLtSessDisconnect=vLtSessDisconnect, vProfTcpPort=vProfTcpPort, vTcpSendQSize=vTcpSendQSize, vTelSessDisconnect=vTelSessDisconnect, vTcpTimerInterval=vTcpTimerInterval)
mibBuilder.exportSymbols("VCP-PRIVATE-MIB-VER-2", vPortBroadcast=vPortBroadcast, vTcpRcvWinSize=vTcpRcvWinSize, vQueTable=vQueTable, vTelSessRemAddr=vTelSessRemAddr, vRlogSessTable=vRlogSessTable, vLtNodeName=vLtNodeName, vRlogSessRemAddr=vRlogSessRemAddr, vPortSpeed=vPortSpeed, vHostCurrNumber=vHostCurrNumber, vTelSessState=vTelSessState, vSnmpWriteCommunity=vSnmpWriteCommunity, vProfEraseLine=vProfEraseLine, vPortPasswordEnable=vPortPasswordEnable, vPortParityErrs=vPortParityErrs, vProfPadChar=vProfPadChar, vcpSystem=vcpSystem, vTacTable=vTacTable, vArpRetryTimeout=vArpRetryTimeout, vPortDTRwait=vPortDTRwait, vPortPrompt=vPortPrompt, vPortAutoPrompt=vPortAutoPrompt, vPortProfile=vPortProfile, vPortNumber=vPortNumber, vLtSessSvcName=vLtSessSvcName, vcpTcp=vcpTcp, vNsHostname=vNsHostname, vTacEntry=vTacEntry, vTelSessIpPrecedence=vTelSessIpPrecedence, vPortConsoleIndex=vPortConsoleIndex, vProfCrToNet=vProfCrToNet, vLtMaxSvcs=vLtMaxSvcs, vSysBootFilePath=vSysBootFilePath, vPortInactivityTimer=vPortInactivityTimer, vProfCurrNumber=vProfCurrNumber, vProfIntProcess=vProfIntProcess, service=service, dssServerMibs=dssServerMibs, vLtSessNodeName=vLtSessNodeName, vSecEnable=vSecEnable, vLtSessIndex=vLtSessIndex, vRlogSessOrigin=vRlogSessOrigin, dssTelnet=dssTelnet, vPortOverrunErrs=vPortOverrunErrs, vPortState=vPortState, vRlogSessRemTcpPort=vRlogSessRemTcpPort, vPortSignalCheck=vPortSignalCheck, vLtTotalRetranPkts=vLtTotalRetranPkts, vProfCrFromTerm=vProfCrFromTerm, vcpQueue=vcpQueue, vPortInFlowState=vPortInFlowState, vProfRemoteBinary=vProfRemoteBinary, vProfRemoteEcho=vProfRemoteEcho, vSecGroups=vSecGroups, vSysMaintenancePassword=vSysMaintenancePassword, vIpMaxInterfaces=vIpMaxInterfaces, vPortPrefSvc=vPortPrefSvc, vProfDataMark=vProfDataMark, vNsAllowLowerCase=vNsAllowLowerCase, vPortLoginPassword=vPortLoginPassword, vPortTrnChars=vPortTrnChars, vTelSessAttention=vTelSessAttention, vSysSecondaryBoot=vSysSecondaryBoot, vLtSessTable=vLtSessTable, vLtNodeGroups=vLtNodeGroups, vLtMcastEnable=vLtMcastEnable, vSysRemoteBoot=vSysRemoteBoot, vPortModemControl=vPortModemControl, vProfEntry=vProfEntry, vSecEntry=vSecEntry, vTelSessNullPass=vTelSessNullPass, vTelSessCurrNumber=vTelSessCurrNumber, vProfBreak=vProfBreak, LtGroupList=LtGroupList, vNsTTL=vNsTTL, vPortHandshake=vPortHandshake, vTacHostname=vTacHostname, server=server, vLtReqAccepted=vLtReqAccepted, vTelSessOrigin=vTelSessOrigin, vPortUserName=vPortUserName, vSnmpReadWriteCommunity=vSnmpReadWriteCommunity, vTelSessRemTcpPort=vTelSessRemTcpPort, vQueCurrNumber=vQueCurrNumber, vSvcVirtualText=vSvcVirtualText, vProfIpPrecedence=vProfIpPrecedence)
